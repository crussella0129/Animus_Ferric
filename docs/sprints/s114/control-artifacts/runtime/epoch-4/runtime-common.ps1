...
function ConvertTo-UtcIso8601 {
    ...
    elseif ($Value -is [DateTime]) {
        ...
        if ($dateTime.Kind -eq [DateTimeKind]::Unspecified) {
            throw 'date-time value has no explicit UTC or local kind'
        }
        ...
        if ($text -cnotmatch (
            ...
            throw 'date-time text is not explicit-offset RFC3339'
        }
...
function Test-RecoveryPlanIdentity {
    [CmdletBinding()]
    param([AllowNull()][Parameter(Mandatory = $true)]$Plan)
    ...
    $anchors = @(
        ...
        $Plan.operation.attempt,
        $Plan.operation.attestation,
    ...
    $anchorShapesPassed = @($anchors | Where-Object {
        [string]::IsNullOrWhiteSpace([string]$_.relative_path) -or
        [System.IO.Path]::IsPathRooted([string]$_.relative_path) -or
        [string]$_.relative_path -match '(^|[\\/])\.{1,2}([\\/]|$)' -or
        [UInt64]$_.bytes -eq 0 -or
        [string]$_.sha256 -cnotmatch '^[0-9a-f]{64}$'
    }).Count -eq 0

    $null -ne $terminal -and
        $Plan.schema -ceq 'animus-ferric-runtime-recovery-plan-v4' -and
        $Plan.task -ceq 'T-11409' -and
        ...
        $Plan.repository_commit_before_epoch_4_controls -ceq
        ...
        $Plan.operation.id -ceq 'r04-publish-e03-01-q4-32768' -and
        $Plan.operation.coordinate -ceq 'e03-01-q4-32768' -and
        $Plan.operation.source_attempt_schema -ceq
            'animus-ferric-runtime-attempt-v3' -and
        $Plan.operation.source_raw_relative_path -ceq
            'target/s114-experiment/runtime-epoch-3/smoke/e03-01-q4-32768' -and
        $Plan.operation.destination_relative_path -ceq
            'docs/sprints/s114/control-artifacts/runtime/epoch-3/attempts/e03-01-q4-32768' -and
        [int]$Plan.operation.exact_manifest_entries -eq 49 -and
        $Plan.operation.manifest.relative_path -ceq
            'target/s114-experiment/runtime-epoch-3/smoke/e03-01-q4-32768/files.sha256' -and
        $Plan.operation.attempt.relative_path -ceq
            'target/s114-experiment/runtime-epoch-3/smoke/e03-01-q4-32768/attempt.json' -and
        $Plan.operation.attestation.relative_path -ceq
            'target/s114-experiment/runtime-epoch-3/smoke/e03-01-q4-32768/attestation.json' -and
        $Plan.epoch_3.control_manifest.relative_path -ceq
            'docs/sprints/s114/control-artifacts/runtime/epoch-3/control-inputs.json' -and
        $Plan.epoch_3.control_digest.relative_path -ceq
            'docs/sprints/s114/control-artifacts/runtime/epoch-3/control-inputs.sha256' -and
        $Plan.epoch_3.runtime_plan.relative_path -ceq
            'docs/sprints/s114/control-artifacts/runtime/epoch-3/runtime-plan.json' -and
        $Plan.epoch_3.runtime_self_test.relative_path -ceq
            'docs/sprints/s114/control-artifacts/runtime/epoch-3/runtime-self-test.json' -and
        $Plan.epoch_3.control_manifest_digest_line -ceq
            "$([string]$Plan.epoch_3.control_manifest.sha256)  control-inputs.json" -and
...
function Get-EpochFourStaticControlNames {
    @(
        '.gitattributes',
        'README.md',
        'runtime-plan.json',
