...
try {
$publicationArtifactDir = $PSScriptRoot
. (Join-Path $publicationArtifactDir 'runtime-common.ps1')
$repoRoot = Get-RepositoryRoot -ArtifactDirectory $publicationArtifactDir
$recoveryPlanPath = Join-Path $publicationArtifactDir 'runtime-plan.json'
$recoveryPlan = Get-Content -Raw -LiteralPath $recoveryPlanPath |
    ConvertFrom-Json -DateKind String
$rawSourceAnchorPath = Join-Path $publicationArtifactDir `
    'raw-source-anchor.json'
$rawSourceAnchor = Get-Content -Raw -LiteralPath $rawSourceAnchorPath |
    ConvertFrom-Json -DateKind String
$resolvedAttempt = (Resolve-Path -LiteralPath $AttemptPath).Path
$errors = [System.Collections.Generic.List[string]]::new()

if (-not (Test-RecoveryPlanIdentity -Plan $recoveryPlan)) {
    $errors.Add('runtime recovery plan does not declare the frozen epoch-4 protocol')
}
if ($rawSourceAnchor.schema -cne
        'animus-ferric-runtime-raw-source-anchor-v1' -or
    $rawSourceAnchor.task -cne 'T-11409' -or
    $rawSourceAnchor.operation_id -cne [string]$recoveryPlan.operation.id -or
    [int]$rawSourceAnchor.execution_epoch -ne 3 -or
    [int]$rawSourceAnchor.publication_epoch -ne 4 -or
    $rawSourceAnchor.source_relative_path -cne
        [string]$recoveryPlan.operation.source_raw_relative_path -or
    $rawSourceAnchor.destination_relative_path -cne
        [string]$recoveryPlan.operation.destination_relative_path -or
    [int]$rawSourceAnchor.manifest.entry_count -ne
        [int]$recoveryPlan.operation.exact_manifest_entries -or
    [UInt64]$rawSourceAnchor.manifest.bytes -ne
        [UInt64]$recoveryPlan.operation.manifest.bytes -or
    $rawSourceAnchor.manifest.sha256 -cne
        [string]$recoveryPlan.operation.manifest.sha256 -or
    [UInt64]$rawSourceAnchor.selected.attempt.bytes -ne
        [UInt64]$recoveryPlan.operation.attempt.bytes -or
    $rawSourceAnchor.selected.attempt.sha256 -cne
        [string]$recoveryPlan.operation.attempt.sha256 -or
    [UInt64]$rawSourceAnchor.selected.attestation.bytes -ne
        [UInt64]$recoveryPlan.operation.attestation.bytes -or
    $rawSourceAnchor.selected.attestation.sha256 -cne
        [string]$recoveryPlan.operation.attestation.sha256 -or
    @($rawSourceAnchor.files).Count -ne
        [int]$recoveryPlan.operation.exact_manifest_entries) {
    $errors.Add('raw-source anchor identity differs from the recovery plan')
}
$artifactDir = Resolve-SafeRelativePath -Root $repoRoot `
    -RelativePath ([string]$recoveryPlan.source_artifact_relative_path)
$planPath = Resolve-SafeRelativePath -Root $repoRoot `
    -RelativePath ([string]$recoveryPlan.epoch_3.runtime_plan.relative_path)
$planItem = Get-Item -LiteralPath $planPath -Force
if ($planItem.Attributes.HasFlag([System.IO.FileAttributes]::ReparsePoint) -or
    [UInt64]$planItem.Length -ne
        [UInt64]$recoveryPlan.epoch_3.runtime_plan.bytes -or
    (Get-Sha256Lower -Path $planPath) -cne
        [string]$recoveryPlan.epoch_3.runtime_plan.sha256) {
    $errors.Add('epoch-3 runtime plan differs from its recovery anchor')
}
...
function Get-StrictUtcTimestamp {
    ...
    $normalized = try {
        ConvertTo-UtcIso8601 -Value $Value
    }
    catch {
        $null
    }
    $passed = $null -ne $normalized -and
    ...
    [pscustomobject]@{
        passed = $passed
        normalized = if ($passed) { [string]$normalized } else { $null }
        instant = $instant
    }
}

function Test-RecoveryFileAnchor {
    param(
    ...
    if ($null -eq $Anchor) {
        Add-Error "$Label anchor is absent"
        return $false
    }
    $path = try {
        Resolve-SafeRelativePath -Root $repoRoot `
            -RelativePath ([string]$Anchor.relative_path)
    }
    catch {
        Add-Error "$Label anchor path is unsafe"
        return $false
    }
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        Add-Error "$Label anchored file is absent"
        return $false
    }
    ...
    if (-not $passed) {
        Add-Error "$Label anchored file differs"
    }
    $passed
}
...
function Test-ExchangeFile {
    ...
    $startedParse = Get-StrictUtcTimestamp -Value $startedText
    $completedParse = Get-StrictUtcTimestamp -Value $completedText
    $started = [DateTimeOffset]$startedParse.instant
    $completed = [DateTimeOffset]$completedParse.instant
    if (-not $startedParse.passed -or
        -not $completedParse.passed -or
...
function Test-MemorySnapshot {
    ...
    $capturedParse = Get-StrictUtcTimestamp -Value $capturedAt
    if (-not $capturedParse.passed) {
...
function Test-ProcessRecord {
    ...
    $recordStartedParse = Get-StrictUtcTimestamp -Value (
        Get-OptionalProperty -Value $Record -Name 'started_at_utc'
    )
    $recordCompletedParse = Get-StrictUtcTimestamp -Value (
        Get-OptionalProperty -Value $Record -Name 'completed_at_utc'
    )
    $recordStarted = [DateTimeOffset]$recordStartedParse.instant
    $recordCompleted = [DateTimeOffset]$recordCompletedParse.instant
    $recordDuration = Get-OptionalProperty -Value $Record -Name 'duration_ms'
    if (-not $recordStartedParse.passed -or
        -not $recordCompletedParse.passed -or
...
$publicationControlManifestPath = Join-Path $publicationArtifactDir `
    'control-inputs.json'
$publicationControlDigestPath = Join-Path $publicationArtifactDir `
    'control-inputs.sha256'
$controlManifest = $null
$publicationControlManifest = $null
$controlAnchorMode = 'epoch_4_frozen_recovery'
...
$selfTestRoot = [System.IO.Path]::GetFullPath(
    (Join-Path $repoRoot 'target/s114-experiment/runtime-epoch-4-selftest')
).TrimEnd(
    [System.IO.Path]::DirectorySeparatorChar,
    [System.IO.Path]::AltDirectorySeparatorChar
) + [System.IO.Path]::DirectorySeparatorChar
$sourceRawPath = Resolve-SafeRelativePath -Root $repoRoot `
    -RelativePath ([string]$recoveryPlan.operation.source_raw_relative_path)
$destinationPath = Resolve-SafeRelativePath -Root $repoRoot `
    -RelativePath ([string]$recoveryPlan.operation.destination_relative_path)
$publicationManifestExists = Test-Path `
    -LiteralPath $publicationControlManifestPath -PathType Leaf
$publicationDigestExists = Test-Path `
    -LiteralPath $publicationControlDigestPath -PathType Leaf
$controlsAbsent = -not $publicationManifestExists -and
    -not $publicationDigestExists
$isSelfTestAttempt = $resolvedAttempt.StartsWith(
    $selfTestRoot,
    [System.StringComparison]::OrdinalIgnoreCase
)
$isRawSourceAttempt = $resolvedAttempt.Equals(
    $sourceRawPath,
    [System.StringComparison]::OrdinalIgnoreCase
)
$isDestinationAttempt = $resolvedAttempt.Equals(
    $destinationPath,
    [System.StringComparison]::OrdinalIgnoreCase
)
$publicationStageParent = [System.IO.Path]::GetFullPath(
    (Join-Path $repoRoot 'target/s114-experiment/recovery-stage')
)
$attemptParent = Split-Path -Parent $resolvedAttempt
$stageOwnerParent = Split-Path -Parent $attemptParent
$isPublicationStageAttempt =
    (Test-Path -LiteralPath $publicationStageParent -PathType Container) -and
    -not (Get-Item -LiteralPath $publicationStageParent -Force).Attributes.HasFlag(
        [System.IO.FileAttributes]::ReparsePoint
    ) -and
    (Split-Path -Leaf $resolvedAttempt) -ceq
        [string]$recoveryPlan.operation.coordinate -and
    (Split-Path -Leaf $attemptParent) -cmatch '^[0-9a-f]{32}$' -and
    [System.IO.Path]::GetFullPath($stageOwnerParent).Equals(
        $publicationStageParent,
        [System.StringComparison]::OrdinalIgnoreCase
    ) -and
    -not (Get-Item -LiteralPath $resolvedAttempt -Force).Attributes.HasFlag(
        [System.IO.FileAttributes]::ReparsePoint
    ) -and
    -not (Get-Item -LiteralPath $attemptParent -Force).Attributes.HasFlag(
        [System.IO.FileAttributes]::ReparsePoint
    )
$deferLiveModelHash =
    ...
    $isSelfTestAttempt
if ($publicationManifestExists -ne $publicationDigestExists) {
    throw 'epoch-4 control manifest and digest must be both present or both absent'
}
if (@(
        [bool]$UnfrozenRecoverySource,
        [bool]$RecoveryPublicationStage,
        [bool]$DeferLiveModelHashToFreeze
    ).Where({ $_ }).Count -gt 1) {
    throw 'recovery verifier modes are mutually exclusive'
}
if ($controlsAbsent) {
    if ($RecoveryPublicationStage) {
        throw 'publication-stage verification requires frozen epoch-4 controls'
    }
    elseif ($UnfrozenRecoverySource) {
        if (-not $isRawSourceAttempt) {
            throw 'unfrozen recovery is restricted to the exact raw source attempt'
        }
        $controlAnchorMode = 'unfrozen_raw_recovery'
    }
    elseif ($DeferLiveModelHashToFreeze) {
        if (-not $deferLiveModelHash) {
            throw 'model-hash deferral is restricted to epoch-4 self-test attempts'
        }
        $controlAnchorMode = 'unfrozen_epoch_4_self_test'
    }
    else {
        throw 'unfrozen verification requires an explicit recovery or self-test mode'
    }
}
elseif ($UnfrozenRecoverySource -or $DeferLiveModelHashToFreeze) {
    throw 'unfrozen verifier modes are forbidden after epoch-4 control freeze'
}
elseif ($RecoveryPublicationStage) {
    if (-not $isPublicationStageAttempt) {
        throw 'publication-stage path does not have the exact owned stage shape'
    }
    $controlAnchorMode = 'epoch_4_frozen_publication_stage'
}
elseif (-not $isDestinationAttempt) {
    throw 'frozen recovery verification is restricted to the exact destination'
}

[void](Test-RecoveryFileAnchor `
    -Anchor $recoveryPlan.epoch_3.control_manifest `
    -Label 'epoch-3 control manifest')
[void](Test-RecoveryFileAnchor `
    -Anchor $recoveryPlan.epoch_3.control_digest `
    -Label 'epoch-3 control digest')
[void](Test-RecoveryFileAnchor `
    -Anchor $recoveryPlan.epoch_3.runtime_plan `
    -Label 'epoch-3 runtime plan')
[void](Test-RecoveryFileAnchor `
    -Anchor $recoveryPlan.epoch_3.runtime_self_test `
...
if (-not (Test-Path -LiteralPath $controlManifestPath -PathType Leaf) -or
    ...
    Add-Error 'epoch-3 frozen control-input manifest or digest is absent'
}
else {
    $digestLine = (Get-Content -Raw -LiteralPath $controlDigestPath).Trim()
    if ($digestLine -cne
            [string]$recoveryPlan.epoch_3.control_manifest_digest_line -or
        $digestLine -notmatch '^([0-9a-f]{64})  control-inputs\.json$' -or
...
if (-not $controlsAbsent) {
    $publicationDigestLine = (
        Get-Content -Raw -LiteralPath $publicationControlDigestPath
    ).Trim()
    if ($publicationDigestLine -notmatch
            '^([0-9a-f]{64})  control-inputs\.json$' -or
        (Get-Sha256Lower -Path $publicationControlManifestPath) -cne
            $Matches[1]) {
        Add-Error 'epoch-4 frozen control-input digest is invalid'
    }
    else {
        $publicationControlManifest = Get-Content -Raw `
            -LiteralPath $publicationControlManifestPath |
            ConvertFrom-Json -DateKind String
        $freezeTimestamp = Get-StrictUtcTimestamp `
            -Value $publicationControlManifest.frozen_at_utc
        if ($publicationControlManifest.schema -cne
                'animus-ferric-runtime-recovery-control-inputs-v4' -or
            $publicationControlManifest.task -cne 'T-11409' -or
            $publicationControlManifest.operation_id -cne
                [string]$recoveryPlan.operation.id -or
            [int]$publicationControlManifest.execution_epoch -ne 3 -or
            [int]$publicationControlManifest.publication_epoch -ne 4 -or
            $publicationControlManifest.timestamp_protocol -cne
                [string]$recoveryPlan.timestamp_protocol -or
            -not $freezeTimestamp.passed -or
            $publicationControlManifest.runtime_plan_sha256 -cne
                (Get-Sha256Lower -Path $recoveryPlanPath) -or
            $publicationControlManifest.raw_source_anchor_sha256 -cne
                (Get-Sha256Lower -Path (
                    Join-Path $publicationArtifactDir 'raw-source-anchor.json'
                )) -or
            $publicationControlManifest.repository.head_at_freeze -cne
                [string]$recoveryPlan.repository_commit_before_epoch_4_controls -or
            $publicationControlManifest.repository.epoch_4_pre_control_base -cne
                [string]$recoveryPlan.repository_commit_before_epoch_4_controls) {
            Add-Error 'epoch-4 frozen control manifest identity differs'
        }
        if (-not (Test-JsonEquivalent `
                -Left $publicationControlManifest.epoch_3.control_manifest `
                -Right $recoveryPlan.epoch_3.control_manifest) -or
            -not (Test-JsonEquivalent `
                -Left $publicationControlManifest.epoch_3.control_digest `
                -Right $recoveryPlan.epoch_3.control_digest) -or
            -not (Test-JsonEquivalent `
                -Left $publicationControlManifest.epoch_3.runtime_plan `
                -Right $recoveryPlan.epoch_3.runtime_plan) -or
            -not (Test-JsonEquivalent `
                -Left $publicationControlManifest.epoch_3.runtime_self_test `
                -Right $recoveryPlan.epoch_3.runtime_self_test) -or
            $publicationControlManifest.epoch_3.control_manifest_digest_line -cne
                [string]$recoveryPlan.epoch_3.control_manifest_digest_line -or
            [int]$publicationControlManifest.epoch_3.transitive_controls_checked -ne
                @($controlManifest.controls).Count -or
            -not [bool]$publicationControlManifest.epoch_3.passed) {
            Add-Error 'epoch-4 controls do not bind the exact epoch-3 controls'
        }

        $expectedPublicationNames = @(Get-EpochFourStaticControlNames)
        $observedPublicationNames = @(
            $publicationControlManifest.static_controls |
                ForEach-Object { [string]$_.path }
        )
        if (($observedPublicationNames -join "`n") -cne
            ($expectedPublicationNames -join "`n")) {
            Add-Error 'epoch-4 manifest does not name the exact static control set'
        }
        $seenPublicationControls =
            [System.Collections.Generic.HashSet[string]]::new(
                [System.StringComparer]::OrdinalIgnoreCase
            )
        foreach ($control in @($publicationControlManifest.static_controls)) {
            $relative = [string]$control.path
            if ($relative -notmatch '^[A-Za-z0-9._-]+$' -or
                -not $seenPublicationControls.Add($relative)) {
                Add-Error "unsafe or duplicate epoch-4 control path: $relative"
                continue
            }
            $controlPath = Resolve-SafeRelativePath `
                -Root $publicationArtifactDir -RelativePath $relative
            if (-not (Test-Path -LiteralPath $controlPath -PathType Leaf) -or
                [UInt64](Get-Item -LiteralPath $controlPath).Length -ne
                    [UInt64]$control.bytes -or
                (Get-Sha256Lower -Path $controlPath) -cne
                    [string]$control.sha256) {
                Add-Error "frozen epoch-4 control differs: $relative"
            }
        }

        $publicationSelfTest =
            $publicationControlManifest.runtime_self_test
        $publicationSelfTestPath = Resolve-SafeRelativePath -Root $repoRoot `
            -RelativePath ([string]$publicationSelfTest.relative_path)
        if (-not (Test-Path -LiteralPath $publicationSelfTestPath -PathType Leaf) -or
            [UInt64](Get-Item -LiteralPath $publicationSelfTestPath).Length -ne
                [UInt64]$publicationSelfTest.bytes -or
            (Get-Sha256Lower -Path $publicationSelfTestPath) -cne
                [string]$publicationSelfTest.sha256 -or
            -not [bool]$publicationSelfTest.passed) {
            Add-Error 'epoch-4 runtime self-test differs from frozen controls'
        }
        if ($publicationControlManifest.raw_source.relative_path -cne
                [string]$recoveryPlan.operation.source_raw_relative_path -or
            $publicationControlManifest.raw_source.manifest_sha256 -cne
                [string]$recoveryPlan.operation.manifest.sha256 -or
            [int]$publicationControlManifest.raw_source.entries -ne
                [int]$recoveryPlan.operation.exact_manifest_entries -or
            -not [bool]$publicationControlManifest.raw_source.terminal_facts_passed -or
            -not (Test-JsonEquivalent `
                -Left $publicationControlManifest.model `
                -Right ([pscustomobject][ordered]@{
                    relative_path = [string]$recoveryPlan.model.relative_path
                    bytes = [UInt64]$recoveryPlan.model.bytes
                    sha256 = [string]$recoveryPlan.model.sha256
                    independently_rehashed = $true
                    passed = $true
                })) -or
            $publicationControlManifest.destination.relative_path -cne
                [string]$recoveryPlan.operation.destination_relative_path -or
            -not [bool]$publicationControlManifest.destination.absent_at_freeze -or
            $publicationControlManifest.source_verification.schema -cne
                'animus-ferric-runtime-recovery-verification-v4' -or
            $publicationControlManifest.source_verification.operation_id -cne
                [string]$recoveryPlan.operation.id -or
            -not [bool]$publicationControlManifest.source_verification.passed -or
            [bool]$publicationControlManifest.source_verification.hash_deferral_used) {
            Add-Error 'epoch-4 recovery evidence is not exactly frozen'
        }
    }
}
...
if ($errors.Count -gt 0) {
    $early = [ordered]@{
        schema = 'animus-ferric-runtime-recovery-verification-v4'
        task = 'T-11409'
...
$recoveryAnchorCheck = [ordered]@{
    applicable = -not $isSelfTestAttempt
    passed = $true
    expected_entries = [int]$recoveryPlan.operation.exact_manifest_entries
    observed_entries = [int]$manifestCheck.entries
}
if (-not $isSelfTestAttempt) {
    $manifestItem = Get-Item -LiteralPath $manifestPath -Force
    $attemptItem = Get-Item -LiteralPath $attemptPath -Force
    $attestationPath = Join-Path $resolvedAttempt 'attestation.json'
    $attestationItem = if (Test-Path -LiteralPath $attestationPath -PathType Leaf) {
        Get-Item -LiteralPath $attestationPath -Force
    }
    else { $null }
    $selectedAnchorsPassed =
        -not $manifestItem.Attributes.HasFlag(
            [System.IO.FileAttributes]::ReparsePoint
        ) -and
        [UInt64]$manifestItem.Length -eq
            [UInt64]$recoveryPlan.operation.manifest.bytes -and
        (Get-Sha256Lower -Path $manifestPath) -ceq
            [string]$recoveryPlan.operation.manifest.sha256 -and
        -not $attemptItem.Attributes.HasFlag(
            [System.IO.FileAttributes]::ReparsePoint
        ) -and
        [UInt64]$attemptItem.Length -eq
            [UInt64]$recoveryPlan.operation.attempt.bytes -and
        (Get-Sha256Lower -Path $attemptPath) -ceq
            [string]$recoveryPlan.operation.attempt.sha256 -and
        $null -ne $attestationItem -and
        -not $attestationItem.Attributes.HasFlag(
            [System.IO.FileAttributes]::ReparsePoint
        ) -and
        [UInt64]$attestationItem.Length -eq
            [UInt64]$recoveryPlan.operation.attestation.bytes -and
        (Get-Sha256Lower -Path $attestationPath) -ceq
            [string]$recoveryPlan.operation.attestation.sha256
    ...
    foreach ($line in Get-Content -LiteralPath $manifestPath) {
        if ($line -cmatch '^([0-9a-f]{64})  (.+)$' -and
            -not $manifestByPath.ContainsKey($Matches[2])) {
            $manifestByPath.Add($Matches[2], $Matches[1])
        }
    }
    $anchorPaths = [System.Collections.Generic.HashSet[string]]::new(
        [System.StringComparer]::OrdinalIgnoreCase
    )
    $anchorTreePassed =
        @($rawSourceAnchor.files).Count -eq
            [int]$recoveryPlan.operation.exact_manifest_entries -and
        $manifestByPath.Count -eq
            [int]$recoveryPlan.operation.exact_manifest_entries -and
        $actualPaths.Count -eq
            [int]$recoveryPlan.operation.exact_manifest_entries
    foreach ($entry in @($rawSourceAnchor.files)) {
        $relative = [string]$entry.path
        $entryPath = try {
            Resolve-SafeRelativePath -Root $resolvedAttempt `
                -RelativePath $relative
        }
        catch {
            $null
        }
        if (-not $anchorPaths.Add($relative) -or
            -not $manifestByPath.ContainsKey($relative) -or
            $manifestByPath[$relative] -cne [string]$entry.sha256 -or
            $null -eq $entryPath -or
            -not (Test-Path -LiteralPath $entryPath -PathType Leaf)) {
            $anchorTreePassed = $false
            continue
        }
        $entryItem = Get-Item -LiteralPath $entryPath -Force
        if ($entryItem.Attributes.HasFlag(
                [System.IO.FileAttributes]::ReparsePoint
            ) -or
            [UInt64]$entryItem.Length -ne [UInt64]$entry.bytes) {
            $anchorTreePassed = $false
        }
    }
    $recoveryAnchorCheck.passed =
        $selectedAnchorsPassed -and
        $anchorTreePassed -and
        [int]$manifestCheck.entries -eq
            [int]$recoveryPlan.operation.exact_manifest_entries
    if (-not $recoveryAnchorCheck.passed) {
        Add-Error 'attempt tree differs from the exact recovery source anchors'
    }
}
...
$expectedTerminal = $recoveryPlan.operation.expected_terminal
if (-not $isSelfTestAttempt -and
    ($attempt.coordinate -cne [string]$recoveryPlan.operation.coordinate -or
        $attempt.quant -cne [string]$expectedTerminal.quant -or
        [int]$attempt.context -ne [int]$expectedTerminal.context -or
        $attempt.verdict -cne [string]$expectedTerminal.verdict -or
        [bool]$attempt.evidence_complete -ne
            [bool]$expectedTerminal.evidence_complete -or
        [bool]$attempt.startup.healthy -ne
            [bool]$expectedTerminal.startup_healthy -or
        [bool]$attempt.attestation.passed -ne
            [bool]$expectedTerminal.attestation_passed -or
        [bool]$attempt.smoke.passed -ne
            [bool]$expectedTerminal.smoke_passed -or
        [bool]$attempt.throughput.passed -ne
            [bool]$expectedTerminal.throughput_passed -or
        [bool]$attempt.teardown.passed -ne
            [bool]$expectedTerminal.teardown_passed -or
        [double]$attempt.throughput.median_decoded_tokens_per_second -ne
            [double]$expectedTerminal.median_decoded_tokens_per_second)) {
    Add-Error 'attempt terminal facts differ from the recovery plan'
}
...
$attemptStartedParse = Get-StrictUtcTimestamp -Value $attempt.started_at_utc
$attemptCompletedParse = Get-StrictUtcTimestamp -Value $attempt.completed_at_utc
$attemptStarted = [DateTimeOffset]$attemptStartedParse.instant
$attemptCompleted = [DateTimeOffset]$attemptCompletedParse.instant
$attemptChronologyPassed =
    $attemptStartedParse.passed -and
    $attemptCompletedParse.passed -and
...
if (-not (Test-Path -LiteralPath $journalPath -PathType Leaf)) {
    ...
    foreach ($journalRow in $journalRows) {
        $journalTimeParse = Get-StrictUtcTimestamp -Value $journalRow.at_utc
        $journalTime = [DateTimeOffset]$journalTimeParse.instant
        ...
        if ($journalRow.schema -ne 'animus-ferric-runtime-journal-row-v1' -or
            ...
            -not $journalTimeParse.passed -or
    ...
    if ($null -eq $coordinate.predecessor) {
        ...
        if (-not (Test-Path -LiteralPath $predecessorPath -PathType Leaf) -or
            ...
            $predecessorValidation = try {
                $predecessorValidationProcess.stdout |
                    ConvertFrom-Json -DateKind String
            }
            catch {
                $null
...
if ($null -ne $teardownEvidence) {
    ...
    function Add-ExpectedLiveWrapperRecord {
        ...
        if ($null -ne $ProcessRecord -and
            [bool](Get-OptionalProperty -Value $ProcessRecord `
                -Name 'post_process_alive')) {
            $wrapperStarted = Get-StrictUtcTimestamp -Value (
                Get-OptionalProperty -Value $ProcessRecord `
                    -Name 'started_at_utc'
            )
            if (-not $wrapperStarted.passed) {
                Add-Error "$Source live-wrapper timestamp is invalid"
            }
            $expectedLiveWrapperRecords.Add([ordered]@{
                source = $Source
                pid = [UInt32]$ProcessRecord.pid
                file = [string]$ProcessRecord.file
                started_at_utc = $wrapperStarted.normalized
            })
        }
    }
    ...
    if ($wrapperCleanup.Count -ne $expectedLiveWrapperRecords.Count) {
        ...
        for ($wrapperIndex = 0; $wrapperIndex -lt $wrapperCleanup.Count;
            ...
            $cleanupExpectedStarted = Get-StrictUtcTimestamp -Value (
                Get-OptionalProperty -Value $cleanup `
                    -Name 'expected_started_at_utc'
            )
            $cleanupIdentityPassed =
                ...
                $cleanupExpectedStarted.passed -and
                $cleanupExpectedStarted.normalized -ceq
...
if ($attempt.startup.healthy) {
    ...
    $preflightCapturedParse = Get-StrictUtcTimestamp `
        -Value $processCreationBinding.preflight_captured_utc
    $attestationCapturedParse = Get-StrictUtcTimestamp `
        -Value $processCreationBinding.attestation_captured_utc
    $preflightCaptured = [DateTimeOffset]$preflightCapturedParse.instant
    $attestationCaptured = [DateTimeOffset]$attestationCapturedParse.instant
    $processEvidenceChronologyPassed =
        $preflightCapturedParse.passed -and
        $attestationCapturedParse.passed -and
        $attemptChronologyPassed -and
    ...
    if (-not $processEvidenceChronologyPassed) {
        Add-Error (
            'process attestation timestamps are outside the attempt chronology ' +
            "(attempt=$attemptChronologyPassed; " +
            "preflight_parsed=$($preflightCapturedParse.passed); " +
            "attestation_parsed=$($attestationCapturedParse.passed); " +
            "start=$($attemptStarted.ToString('o')); " +
            "preflight=$($preflightCaptured.ToString('o')); " +
            "attestation=$($attestationCaptured.ToString('o')); " +
            "completed=$($attemptCompleted.ToString('o')))"
        )
    }
...
$report = [ordered]@{
    ...
    recovery_anchor = $recoveryAnchorCheck
...
exit 0
    ...
    [ordered]@{
        schema = 'animus-ferric-runtime-recovery-verification-v4'
        task = 'T-11409'
        operation_id = 'r04-publish-e03-01-q4-32768'
        execution_epoch = 3
        publication_epoch = 4
        source_attempt_schema = 'animus-ferric-runtime-attempt-v3'
        timestamp_protocol =
            'powershell-json-datekind-string-rfc3339-v1'
        control_epoch = 3
