...
if (-not (Test-Path -LiteralPath $controlManifestPath -PathType Leaf) -or
        ...
        $controlManifest = [pscustomobject]@{
            binaries = [pscustomobject]@{
                ...
                llama_server = [pscustomobject]@{
                    ...
                    device_output = @(Invoke-BoundedTextProcess `
    ...
    if ($digestLine -notmatch '^([0-9a-f]{64})  control-inputs\.json$' -or
        ...
        foreach ($control in @($controlManifest.controls)) {
            $controlPath = Join-Path $artifactDir ([string]$control.path)
            if (-not (Test-Path -LiteralPath $controlPath -PathType Leaf) -or
                (Get-Sha256Lower -Path $controlPath) -ne $control.sha256) {
                Add-Error "frozen runtime control differs: $($control.path)"
            }
        }
    }
}
...
if ($errors.Count -gt 0) {
    $early = [ordered]@{
        schema = 'animus-ferric-runtime-verification-v1'
...
if ($attempt.schema -ne 'animus-ferric-runtime-attempt-v1') {
    ...
    if ($null -eq $coordinate.predecessor) {
        ...
        if (-not (Test-Path -LiteralPath $predecessorPath -PathType Leaf) -or
            ...
            $predecessorManifestCheck = Test-HashManifest `
                -Root $predecessorDir -ManifestPath $predecessorManifest
...
if ($null -ne $commonModelSpec) {
    ...
    $commonLaunchPassed =
        ...
        $commonLaunch.schema -eq 'animus-ferric-runtime-launch-v1' -and
        ...
        [string]$commonLaunch.environment.Path -eq "$commonLlamaBin;$env:Path" -and
        [System.IO.Path]::GetFullPath(
            [string]$commonLaunch.environment.LLAMA_ARG_LOG_FILE
        ).Equals(
            [System.IO.Path]::GetFullPath(
                (Join-Path $repoRoot `
                    "target/s114-experiment/smoke/$($attempt.coordinate)/server.log")
            ),
            [System.StringComparison]::OrdinalIgnoreCase
        )
    ...
    $commonPreflightPassed =
        ...
        $commonPreflight.schema -eq 'animus-ferric-runtime-preflight-v1' -and
        ...
        (@($commonPreflight.llama_server.devices) -join "`n") -eq
            (@($controlManifest.binaries.llama_server.device_output) -join "`n") -and
        ...
        [UInt64](Get-Item -LiteralPath $commonModelPath).Length -eq
            [UInt64]$commonModelSpec.bytes -and
        (Get-Sha256Lower -Path $commonModelPath) -eq
            $commonModelSpec.sha256 -and
...
if ($attempt.startup.healthy) {
    ...
    if ($null -ne $launchEvidence) {
        $launchFieldsPassed =
            $launchEvidence.schema -eq 'animus-ferric-runtime-launch-v1' -and
        ...
        $expectedRawAttemptRoot = Join-Path $repoRoot `
            "target/s114-experiment/smoke/$($attempt.coordinate)"
        if (-not [string]$launchPath.Equals(
                "$llamaBinForEvidence;$env:Path",
                [System.StringComparison]::OrdinalIgnoreCase
            ) -or
    ...
    $liveModelIdentityPassed =
        ...
        (Get-Sha256Lower -Path $modelPathForEvidence) -eq
            $modelSpecForEvidence.sha256
    ...
    $defaultTemplateKwargs = Get-OptionalProperty -Value $propsJson `
        -Name 'default_template_kwargs'
    $preserveReasoningKwarg = Get-OptionalProperty `
        -Value $defaultTemplateKwargs -Name 'preserve_reasoning'
    $preserveThinkingKwarg = Get-OptionalProperty `
        -Value $defaultTemplateKwargs -Name 'preserve_thinking'
    $enableThinkingKwarg = Get-OptionalProperty `
        -Value $defaultTemplateKwargs -Name 'enable_thinking'
    $preserveReasoningEnabled =
        $preserveReasoningKwarg -eq $true -or
        $preserveThinkingKwarg -eq $true
    ...
    if ($null -ne $preflightEvidence) {
        $preflightPassed =
            $preflightEvidence.schema -eq 'animus-ferric-runtime-preflight-v1' -and
            ...
            (@($preflightEvidence.llama_server.version) -join "`n") -eq
                (@($controlManifest.binaries.llama_server.version_output) -join "`n") -and
            (@($preflightEvidence.llama_server.devices) -join "`n") -eq
                (@($controlManifest.binaries.llama_server.device_output) -join "`n") -and
    ...
    $actualProcessArgv = Split-SimpleWindowsCommandLine `
        -CommandLine ([string]$attempt.attestation.process.command_line)
    $expectedProcessArgv = @(
        [string]$attempt.attestation.process.executable_path
    ) + @($expectedLlamaArguments | Select-Object -Skip 1)
    ...
    $processAndListenerPassed =
        $attempt.attestation.process.executable_sha256 -eq
            $plan.llama_cpp.expected_server_sha256 -and
        [System.IO.Path]::GetFileName(
            [string]$attempt.attestation.process.executable_path
        ) -eq 'llama-server.exe' -and
        [System.IO.Path]::GetFullPath(
            [string]$attempt.attestation.process.executable_path
        ).Equals(
            [System.IO.Path]::GetFullPath(
        ...
        ($actualProcessArgv -join "`n") -eq ($expectedProcessArgv -join "`n") -and
        $listenerRecordsPassed
    $endpointValuesPassed =
        ...
        $preserveReasoningEnabled -and
        $enableThinkingKwarg -eq $true
    $effectiveValuesPassed =
        ...
        $attempt.attestation.effective.preserve_reasoning_evidence_source -eq
            'props.default_template_kwargs' -and
