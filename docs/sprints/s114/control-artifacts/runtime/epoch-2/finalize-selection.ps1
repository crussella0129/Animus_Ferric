[CmdletBinding()]
param(
    [ValidateSet('e02-03-q3-32768', 'e02-04-q3-16384')]
    [string]$Q3AttemptId
)
...
function Assert-FrozenControls {
    ...
    if ($controlManifest.schema -cne
        ...
        -not (Test-JsonEquivalent -Left @($controlManifest.epoch_1_anchors) `
            -Right @($plan.recovery.epoch_1_anchors)) -or
...
function Invoke-Q4GateVerification {
    ...
    if ([int]$processResult.exit_code -ne 0 -or
        $report.schema -cne 'animus-ferric-q4-gate-verification-v2' -or
        $report.task -cne 'T-11409' -or
        [int]$report.control_epoch -ne 2 -or
...
function Get-VerifiedAttempt {
    ...
    if ([int]$processResult.exit_code -ne 0 -or
        $report.schema -cne 'animus-ferric-runtime-verification-v2' -or
        $report.task -cne 'T-11409' -or
        [int]$report.control_epoch -ne 2 -or
        $report.attestation_protocol -cne
            [string]$plan.template_attestation.protocol -or
    ...
    if ($attempt.schema -cne 'animus-ferric-runtime-attempt-v2' -or
...
if (Test-Path -LiteralPath $finalDir) {
    throw 'runtime/epoch-2/final already exists and will not be overwritten'
}
...
if ($q4Ids.Count -lt 1 -or $q4Ids.Count -gt 2 -or
    $q4Ids[0] -cne 'e02-01-q4-32768' -or
    ($q4Ids.Count -eq 2 -and $q4Ids[1] -cne 'e02-02-q4-16384') -or
...
elseif ($q4Derivation.q4_verdict -ceq 'non_viable') {
    if ([bool]$q4Derivation.q3_fallback_authorized) {
        $q3PrimaryProbe = Get-VerifiedAttempt -Id 'e02-03-q3-32768'
        $expectedIds.Add('e02-03-q3-32768')
        $q3TerminalId = 'e02-03-q3-32768'
        if ($q3PrimaryProbe.attempt.failure_classification -ceq
                'startup_memory_pressure') {
            Get-VerifiedAttempt -Id 'e02-04-q3-16384' | Out-Null
            $expectedIds.Add('e02-04-q3-16384')
            $q3TerminalId = 'e02-04-q3-16384'
        }
...
$selection = [ordered]@{
    schema = 'animus-ferric-qwen38-selection-v2'
    control_epoch = 2
...
$verification = [ordered]@{
    schema = 'animus-ferric-runtime-tests-v2'
    task = 'T-11409'
    control_epoch = 2
    attestation_protocol = [string]$plan.template_attestation.protocol
...
try {
    ...
    $manifest = [ordered]@{
        schema = 'animus-ferric-runtime-artifact-manifest-v2'
        control_epoch = 2
        attestation_protocol = [string]$plan.template_attestation.protocol
        task = 'T-11409'
        ...
        excluded_self_paths = @(
            'epoch-2/final/artifact-manifest.json',
            'epoch-2/final/artifact-manifest.sha256'
        )
