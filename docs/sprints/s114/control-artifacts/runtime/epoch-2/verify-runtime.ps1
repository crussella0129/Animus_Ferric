...
if (-not (Test-RuntimePlanIdentity -Plan $plan)) {
    $errors.Add('runtime plan does not declare the frozen epoch-2 protocol')
}
    ...
    if ($digestLine -notmatch '^([0-9a-f]{64})  control-inputs\.json$' -or
        ...
        if ($controlManifest.schema -cne
            ...
            -not (Test-JsonEquivalent `
                -Left @($controlManifest.epoch_1_anchors) `
                -Right @($plan.recovery.epoch_1_anchors))) {
            Add-Error 'frozen control-input manifest is not epoch 2'
        }
...
if ($errors.Count -gt 0) {
    $early = [ordered]@{
        schema = 'animus-ferric-runtime-verification-v2'
        task = 'T-11409'
        control_epoch = 2
...
if ($attempt.schema -ne 'animus-ferric-runtime-attempt-v2' -or
    [int]$attempt.control_epoch -ne 2 -or
    $attempt.task -cne 'T-11409' -or
    $attempt.attestation_protocol -ne
        [string]$plan.template_attestation.protocol) {
...
if ($null -ne $commonModelSpec) {
    ...
    $commonLaunchPassed =
        ...
        $commonLaunch.schema -eq 'animus-ferric-runtime-launch-v2' -and
        [int]$commonLaunch.control_epoch -eq 2 -and
    ...
    $commonPreflightPassed =
        ...
        $commonPreflight.schema -eq 'animus-ferric-runtime-preflight-v2' -and
        $commonPreflight.task -ceq 'T-11409' -and
        [int]$commonPreflight.control_epoch -eq 2 -and
...
if ($attempt.startup.healthy) {
    ...
    $attestationSchemaPassed =
        ...
        $attempt.attestation.schema -eq
            'animus-ferric-managed-server-attestation-v2' -and
        [int]$attempt.attestation.control_epoch -eq 2 -and
        $attempt.attestation.attestation_protocol -eq
            [string]$plan.template_attestation.protocol
    if (-not $attestationSchemaPassed) {
        Add-Error 'managed-server attestation does not declare epoch-2 semantics'
    }
    ...
    if ($null -ne $launchEvidence) {
        $launchFieldsPassed =
            $launchEvidence.schema -eq 'animus-ferric-runtime-launch-v2' -and
            [int]$launchEvidence.control_epoch -eq 2 -and
    ...
    if ($null -ne $preflightEvidence) {
        $preflightPassed =
            $preflightEvidence.schema -eq 'animus-ferric-runtime-preflight-v2' -and
            $preflightEvidence.task -ceq 'T-11409' -and
            [int]$preflightEvidence.control_epoch -eq 2 -and
