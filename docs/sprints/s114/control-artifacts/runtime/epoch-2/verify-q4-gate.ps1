...
if (-not (Test-RuntimePlanIdentity -Plan $plan)) {
    $errors.Add('runtime plan is not the epoch-2 recovery plan')
}
...
$primary = Get-VerifiedAttempt -Id 'e02-01-q4-32768'
if ($null -ne $primary) {
    $chain.Add($primary)
}
$requiresRetry = $null -ne $primary -and
    $primary.attempt.failure_classification -eq 'startup_memory_pressure'
$retryPath = Join-Path $attemptsRoot 'e02-02-q4-16384'
if ($requiresRetry) {
    $retry = Get-VerifiedAttempt -Id 'e02-02-q4-16384'
    if ($null -ne $retry) {
        $chain.Add($retry)
    }
}
...
$actualQ4Ids = if (Test-Path -LiteralPath $attemptsRoot) {
    @(
        Get-ChildItem -LiteralPath $attemptsRoot -Directory -Force |
            Where-Object { $_.Name -match '^e02-\d{2}-q4-' } |
...
if (-not $DeriveOnly) {
    ...
    if ($null -ne $gate) {
        if ($gate.schema -ne 'animus-ferric-qwen38-viability-v2' -or
            $gate.task -cne 'T-11409' -or
            [int]$gate.control_epoch -ne 2 -or
...
$report = [ordered]@{
    schema = 'animus-ferric-q4-gate-verification-v2'
    task = 'T-11409'
    control_epoch = 2
