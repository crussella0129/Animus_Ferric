...
if (-not (Test-RuntimePlanIdentity -Plan $plan)) {
    throw 'runtime plan does not declare the epoch-2 recovery protocol'
}
...
if ($repositoryHead -cne
    ...
    throw 'repository HEAD differs from the declared epoch-2 pre-control base'
}
...
$requiredSelfTests = @(
    ...
    'final_manifest_covers_both_runtime_epochs',
    'final_manifest_rejects_external_stage',
    'epoch_1_anchor_mismatch_rejected',
    'epoch_1_protection_anchor_mismatch_rejected',
...
$selfTestPassed =
    $selfTest.schema -eq 'animus-ferric-runtime-self-test-v2' -and
    $selfTest.task -ceq 'T-11409' -and
    [int]$selfTest.control_epoch -eq 2 -and
...
$manifest = [ordered]@{
    schema = 'animus-ferric-runtime-control-inputs-v2'
    task = 'T-11409'
    control_epoch = 2
    ...
    epoch_1_anchors = @($plan.recovery.epoch_1_anchors)
    repository = [ordered]@{
