...
function Assert-FrozenControls {
    ...
    if ($controlManifest.schema -cne
        ...
        -not (Test-JsonEquivalent -Left $controlManifest.recovery_anchors `
        ...
        $controlManifest.repository.prior_evidence_checkpoint -cne
            [string]$plan.recovery.prior_evidence_checkpoint -or
...
function Invoke-Q4GateVerification {
    ...
    if ([int]$processResult.exit_code -ne 0 -or
        $report.schema -cne 'animus-ferric-q4-gate-verification-v3' -or
        $report.task -cne 'T-11409' -or
        [int]$report.control_epoch -ne 3 -or
...
function Get-VerifiedAttempt {
    ...
    if ([int]$processResult.exit_code -ne 0 -or
        $report.schema -cne 'animus-ferric-runtime-verification-v3' -or
        $report.task -cne 'T-11409' -or
        [int]$report.control_epoch -ne 3 -or
        $report.attestation_protocol -cne
            [string]$plan.template_attestation.protocol -or
...
if (Test-Path -LiteralPath $finalDir) {
    throw 'runtime/epoch-3/final already exists and will not be overwritten'
}
...
$selection = [ordered]@{
    schema = 'animus-ferric-qwen38-selection-v3'
    control_epoch = 3
...
$verification = [ordered]@{
    schema = 'animus-ferric-runtime-tests-v3'
    task = 'T-11409'
    control_epoch = 3
    attestation_protocol = [string]$plan.template_attestation.protocol
    process_command_protocol =
...
try {
    ...
    $manifest = [ordered]@{
        schema = 'animus-ferric-runtime-artifact-manifest-v3'
        control_epoch = 3
        attestation_protocol = [string]$plan.template_attestation.protocol
        process_command_protocol =
            [string]$plan.process_command_attestation.protocol
        ...
        excluded_self_paths = @(
            'epoch-3/final/artifact-manifest.json',
            'epoch-3/final/artifact-manifest.sha256'
        )
