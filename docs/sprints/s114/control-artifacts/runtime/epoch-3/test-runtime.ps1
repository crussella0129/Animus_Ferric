...
if (-not (Test-RuntimePlanIdentity -Plan $plan)) {
    throw 'runtime plan is not the epoch-3 T-11409 protocol'
}
...
$testRoot = Join-Path $repoRoot `
    "target/s114-experiment/runtime-epoch-3-selftest/$PID-$stamp"
$base = Join-Path $testRoot 'base/e03-01-q4-32768'
...
function Copy-Case {
    ...
    $destination = Join-Path $parent 'e03-01-q4-32768'
...
function Sync-CaseProcessBinding {
    param([Parameter(Mandatory = $true)][string]$Path)

    $attempt = Read-CaseAttempt -Path $Path
    $processEvidence = $attempt.attestation.process
    $processEvidence.command_binding = Test-BoundWindowsProcessCommand `
        -ExecutablePath ([string]$processEvidence.executable_path) `
        -ExecutableSha256 ([string]$processEvidence.executable_sha256) `
        -CommandLine ([string]$processEvidence.command_line) `
        -FrozenExecutablePath $llamaPath `
        -FrozenExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
        -ExpectedArgv @($llamaArguments)
    ...
    $processEvidence.command_binding
}
...
$liveQ4Identity = [ordered]@{
    path = [string]$modelSpec.relative_path
    bytes = [UInt64]$modelItem.Length
    sha256 = Get-Sha256Lower -Path $modelPath
}
$liveQ4Identity.passed =
    [UInt64]$liveQ4Identity.bytes -eq [UInt64]$modelSpec.bytes -and
    [string]$liveQ4Identity.sha256 -ceq [string]$modelSpec.sha256
...
$preflight = [ordered]@{
    schema = 'animus-ferric-runtime-preflight-v3'
    task = 'T-11409'
    control_epoch = 3
...
$launch = [ordered]@{
    schema = 'animus-ferric-runtime-launch-v3'
    control_epoch = 3
    attestation_protocol = [string]$plan.template_attestation.protocol
    process_command_protocol =
        [string]$plan.process_command_attestation.protocol
...
if ((Get-Sha256Text -Text $chatTemplate) -ne
    ...
    throw 'immutable epoch-1 /props chat template differs from the epoch-3 plan'
}
$propsBody = [ordered]@{
...
$processCommandLine = @($llamaArguments) | ForEach-Object {
    ConvertTo-WindowsCommandLineArgument -Argument ([string]$_)
}
$processCommandText = $processCommandLine -join ' '
$processCommandBinding = Test-BoundWindowsProcessCommand `
    -ExecutablePath $llamaPath `
    -ExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -CommandLine $processCommandText `
    -FrozenExecutablePath $llamaPath `
    -FrozenExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -ExpectedArgv @($llamaArguments)
if (-not $processCommandBinding.passed) {
    throw "base process-command fixture is invalid: $($processCommandBinding.errors -join '; ')"
}
$attestationCapturedUtc = $startedAt.AddSeconds(8).ToString('o')
$processCreationUtc = $startedAt.AddSeconds(3).ToString('o')
$processCreationBinding = Test-ProcessCreationWindow `
    -CreationDateUtc $processCreationUtc `
    -PreflightCapturedUtc ([string]$preflight.captured_at_utc) `
    -AttestationCapturedUtc $attestationCapturedUtc `
    -ToleranceSeconds `
        ([int]$plan.process_command_attestation.creation_time_tolerance_seconds)
if (-not $processCreationBinding.passed) {
    throw "base process-creation fixture is invalid: $($processCreationBinding.errors -join '; ')"
}
$attestation = [ordered]@{
    schema = 'animus-ferric-managed-server-attestation-v3'
    control_epoch = 3
    attestation_protocol = [string]$plan.template_attestation.protocol
    process_command_protocol =
        [string]$plan.process_command_attestation.protocol
    passed = $true
    coordinate = $coordinate
    captured_at_utc = $attestationCapturedUtc
    runfiles = [ordered]@{
    ...
    process = [ordered]@{
        ...
        command_line = $processCommandText
        command_binding = $processCommandBinding
        creation_date_utc = $processCreationUtc
        creation_binding = $processCreationBinding
    }
    listener = [ordered]@{
        owners = @($serverPid)
        records = @([ordered]@{
...
$attempt = [ordered]@{
    schema = 'animus-ferric-runtime-attempt-v3'
    control_epoch = 3
    attestation_protocol = [string]$plan.template_attestation.protocol
    process_command_protocol =
        [string]$plan.process_command_attestation.protocol
...
$deferralProbeParent = Join-Path $repoRoot `
    "target/s114-experiment/runtime-epoch-3-deferral-scope-probe/$PID"
[System.IO.Directory]::CreateDirectory($deferralProbeParent) | Out-Null
$deferralProbePath = Join-Path $deferralProbeParent $coordinate
Copy-Item -LiteralPath $base -Destination $deferralProbePath -Recurse
$deferralProbeProcess = Invoke-PowerShellFileBounded `
...
$deferralProbeReport = try {
    $deferralProbeProcess.stdout | ConvertFrom-Json
}
catch {
    $null
}
$results.Add([ordered]@{
    name = 'model_hash_deferral_scope_is_restricted'
    passed = (
        $deferralProbeProcess.exit_code -ne 0 -and
        $null -ne $deferralProbeReport -and
        -not $deferralProbeReport.passed -and
        @($deferralProbeReport.errors | Where-Object {
            $_ -match 'live model hash deferral is restricted'
        }).Count -ge 1
    )
    evidence = [ordered]@{
        report = $deferralProbeReport
        stderr = $deferralProbeProcess.stderr
    }
})

$baseAttempt = Read-CaseAttempt -Path $base
$liveBinding = $baseAttempt.attestation.process.command_binding
$results.Add([ordered]@{
    name = 'live_basename_argv0_with_frozen_image_passes'
    passed = (
        $valid.exit_code -eq 0 -and
        $valid.parseable -and
        $valid.report.passed -and
        $liveBinding.passed -and
        $liveBinding.argv0_mode -ceq 'declared_bare_alias' -and
        $liveBinding.executable_path_passed -and
        $liveBinding.recorded_executable_sha256_passed -and
        $liveBinding.live_executable_sha256_passed -and
        $liveBinding.argument_tail_passed
    )
    evidence = $liveBinding
})

$absoluteArgvPath = Copy-Case -Name 'absolute-argv0'
$absoluteArgvAttempt = Read-CaseAttempt -Path $absoluteArgvPath
$absoluteArgv = @($llamaPath) + @($llamaArguments | Select-Object -Skip 1)
$absoluteArgvAttempt.attestation.process.command_line =
    Join-WindowsCommandLine -Arguments $absoluteArgv
Write-JsonLf -Path (Join-Path $absoluteArgvPath 'attestation.json') `
    -Value $absoluteArgvAttempt.attestation
Write-CaseAttempt -Path $absoluteArgvPath -Attempt $absoluteArgvAttempt
$absoluteBinding = Sync-CaseProcessBinding -Path $absoluteArgvPath
$absoluteValidation = Invoke-Validator -Path $absoluteArgvPath
$results.Add([ordered]@{
    name = 'frozen_absolute_argv0_passes'
    passed = (
        $absoluteBinding.passed -and
        $absoluteBinding.argv0_mode -ceq 'frozen_absolute_path' -and
        $absoluteValidation.exit_code -eq 0 -and
        $absoluteValidation.parseable -and
        $absoluteValidation.report.passed
    )
    evidence = [ordered]@{
        binding = $absoluteBinding
        validation = $absoluteValidation.report
        stderr = $absoluteValidation.stderr
    }
})

$wrongArgv0Bindings = [System.Collections.Generic.List[object]]::new()
foreach ($wrongArgv0 in @(
    'not-llama-server',
    'llama-server.exe',
    '.\llama-server',
    'C:\foreign\llama-server.exe'
)) {
    $wrongArgv = @($wrongArgv0) + @($llamaArguments | Select-Object -Skip 1)
    $wrongArgv0Bindings.Add(
        (Test-BoundWindowsProcessCommand `
            -ExecutablePath $llamaPath `
            -ExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
            -CommandLine (Join-WindowsCommandLine -Arguments $wrongArgv) `
            -FrozenExecutablePath $llamaPath `
...
$results.Add([ordered]@{
    name = 'unauthorized_argv0_spellings_rejected'
    passed = (
        $wrongArgv0Bindings.Count -eq 4 -and
        @($wrongArgv0Bindings | Where-Object {
            $_.passed -or $_.argv0_passed -or -not $_.argument_tail_passed
        }).Count -eq 0
    )
    evidence = @($wrongArgv0Bindings)
})

$wrongPathBinding = Test-BoundWindowsProcessCommand `
    -ExecutablePath 'C:\foreign\llama-server.exe' `
    -ExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -CommandLine $processCommandText `
    -FrozenExecutablePath $llamaPath `
    -FrozenExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -ExpectedArgv @($llamaArguments)
$wrongHashBinding = Test-BoundWindowsProcessCommand `
    -ExecutablePath $llamaPath `
    ...
    -ExpectedArgv @($llamaArguments)
$results.Add([ordered]@{
    name = 'executable_path_and_hash_tamper_rejected'
    passed = (
        -not $wrongPathBinding.passed -and
        -not $wrongPathBinding.executable_path_passed -and
        -not $wrongPathBinding.live_executable_sha256_passed -and
        -not $wrongHashBinding.passed -and
        $wrongHashBinding.executable_path_passed -and
        -not $wrongHashBinding.recorded_executable_sha256_passed -and
        $wrongHashBinding.live_executable_sha256_passed
    )
    evidence = [ordered]@{
        wrong_path = $wrongPathBinding
        wrong_hash = $wrongHashBinding
    }
})

$tailMutationCases = [System.Collections.Generic.List[object]]::new()
$wrongModelArgv = @($llamaArguments)
$wrongModelArgv[2] = 'C:\foreign\model.gguf'
$tailMutationCases.Add([pscustomobject]@{ argv = @($wrongModelArgv) })
$wrongContextArgv = @($llamaArguments)
$wrongContextArgv[4] = '16384'
$tailMutationCases.Add([pscustomobject]@{ argv = @($wrongContextArgv) })
$wrongPortArgv = @($llamaArguments)
$wrongPortArgv[$wrongPortArgv.Count - 1] = '8081'
$tailMutationCases.Add([pscustomobject]@{ argv = @($wrongPortArgv) })
$tailMutationCases.Add([pscustomobject]@{
    argv = @(
        @($llamaArguments | Select-Object -First 3) +
            @('--unexpected') +
            @($llamaArguments | Select-Object -Skip 3)
    )
})
$tailMutationCases.Add([pscustomobject]@{
    argv = @($llamaArguments | Select-Object -SkipLast 1)
})
$reorderedArgv = @($llamaArguments)
$swap = $reorderedArgv[3]
$reorderedArgv[3] = $reorderedArgv[5]
$reorderedArgv[5] = $swap
$tailMutationCases.Add([pscustomobject]@{ argv = @($reorderedArgv) })
$tailMutationBindings = @($tailMutationCases | ForEach-Object {
    Test-BoundWindowsProcessCommand `
        -ExecutablePath $llamaPath `
        -ExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
        -CommandLine (Join-WindowsCommandLine -Arguments @($_.argv)) `
        -FrozenExecutablePath $llamaPath `
        -FrozenExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
        -ExpectedArgv @($llamaArguments)
})
$results.Add([ordered]@{
    name = 'non_argv0_command_mutations_rejected'
    passed = (
        $tailMutationBindings.Count -eq 6 -and
        @($tailMutationBindings | Where-Object {
            $_.passed -or -not $_.argv0_passed -or $_.argument_tail_passed
        }).Count -eq 0
    )
    evidence = $tailMutationBindings
})

$quotedExpectedArgv = @(
    'llama-server',
    '-m',
    'C:\Program Files\Model Folder\model.gguf',
    '--tag',
    'quote"and\tail\'
)
$quotedBinding = Test-BoundWindowsProcessCommand `
    -ExecutablePath $llamaPath `
    -ExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -CommandLine (Join-WindowsCommandLine -Arguments $quotedExpectedArgv) `
    -FrozenExecutablePath $llamaPath `
    -FrozenExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -ExpectedArgv $quotedExpectedArgv
$boundaryExpectedArgv = @('llama-server', '--value', 'alpha', "beta`ngamma")
$boundaryObservedArgv = @('llama-server', '--value', "alpha`nbeta", 'gamma')
$boundaryBinding = Test-BoundWindowsProcessCommand `
    -ExecutablePath $llamaPath `
    -ExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -CommandLine (Join-WindowsCommandLine -Arguments $boundaryObservedArgv) `
    -FrozenExecutablePath $llamaPath `
    -FrozenExecutableSha256 ([string]$plan.llama_cpp.expected_server_sha256) `
    -ExpectedArgv $boundaryExpectedArgv
$results.Add([ordered]@{
    name = 'windows_quoting_and_argument_boundaries_are_exact'
    passed = (
        $quotedBinding.passed -and
        -not $boundaryBinding.passed -and
        $boundaryBinding.argv0_passed -and
        -not $boundaryBinding.argument_tail_passed -and
        (@($boundaryExpectedArgv) -join "`n") -ceq
            (@($boundaryObservedArgv) -join "`n")
    )
    evidence = [ordered]@{
        quoted = $quotedBinding
        boundary_shift = $boundaryBinding
    }
})

$parserRejections = [System.Collections.Generic.List[object]]::new()
foreach ($badCommandLine in @(
    '',
    ' llama-server --port 8080',
    "llama-server$([char]0)--port 8080",
    '"llama-server --port 8080'
)) {
    $rejected = $false
    $message = $null
    try {
        [void](ConvertFrom-WindowsCommandLine -CommandLine $badCommandLine)
    }
    catch {
        $rejected = $true
        $message = $_.Exception.Message
    }
    $parserRejections.Add([ordered]@{
        sha256 = Get-Sha256Text -Text $badCommandLine
        rejected = $rejected
        error = $message
    })
}
$results.Add([ordered]@{
    name = 'malformed_windows_command_lines_rejected'
    passed = (
        $parserRejections.Count -eq 4 -and
        @($parserRejections | Where-Object { -not $_.rejected }).Count -eq 0
    )
    evidence = @($parserRejections)
})

$tailIntegrationPath = Copy-Case -Name 'process-tail-integration-tamper'
$tailIntegrationAttempt = Read-CaseAttempt -Path $tailIntegrationPath
$tailIntegrationArgv = @($llamaArguments)
$tailIntegrationArgv[4] = '16384'
$tailIntegrationAttempt.attestation.process.command_line =
    Join-WindowsCommandLine -Arguments $tailIntegrationArgv
Write-JsonLf -Path (Join-Path $tailIntegrationPath 'attestation.json') `
    -Value $tailIntegrationAttempt.attestation
Write-CaseAttempt -Path $tailIntegrationPath -Attempt $tailIntegrationAttempt
$tailIntegrationBinding = Sync-CaseProcessBinding -Path $tailIntegrationPath
$tailIntegrationValidation = Invoke-Validator -Path $tailIntegrationPath
$results.Add([ordered]@{
    name = 'process_tail_tamper_rejected_by_verifier'
    passed = (
        -not $tailIntegrationBinding.passed -and
        $tailIntegrationBinding.argv0_passed -and
        -not $tailIntegrationBinding.argument_tail_passed -and
        $tailIntegrationValidation.exit_code -ne 0 -and
        $tailIntegrationValidation.parseable -and
        -not $tailIntegrationValidation.report.passed
    )
    evidence = [ordered]@{
        binding = $tailIntegrationBinding
        validation = $tailIntegrationValidation.report
        stderr = $tailIntegrationValidation.stderr
    }
})

$creationTamperPath = Copy-Case -Name 'process-creation-window-tamper'
$creationTamperAttempt = Read-CaseAttempt -Path $creationTamperPath
$creationTamperAttempt.attestation.process.creation_date_utc =
    $startedAt.AddMinutes(-10).ToString('o')
$creationTamperBinding = Test-ProcessCreationWindow `
    -CreationDateUtc `
        ([string]$creationTamperAttempt.attestation.process.creation_date_utc) `
    -PreflightCapturedUtc ([string]$preflight.captured_at_utc) `
    -AttestationCapturedUtc `
        ([string]$creationTamperAttempt.attestation.captured_at_utc) `
    -ToleranceSeconds `
        ([int]$plan.process_command_attestation.creation_time_tolerance_seconds)
$creationTamperAttempt.attestation.process.creation_binding =
    $creationTamperBinding
Write-JsonLf -Path (Join-Path $creationTamperPath 'attestation.json') `
    -Value $creationTamperAttempt.attestation
Write-CaseAttempt -Path $creationTamperPath -Attempt $creationTamperAttempt
Update-CaseManifest -Path $creationTamperPath
$creationTamperValidation = Invoke-Validator -Path $creationTamperPath
$results.Add([ordered]@{
    name = 'process_creation_window_tamper_rejected'
    passed = (
        -not $creationTamperBinding.passed -and
        @($creationTamperBinding.errors | Where-Object {
            $_ -ceq 'process creation is outside the attested launch window'
        }).Count -eq 1 -and
        $creationTamperValidation.exit_code -ne 0 -and
        $creationTamperValidation.parseable -and
        -not $creationTamperValidation.report.passed
    )
    evidence = [ordered]@{
        binding = $creationTamperBinding
        validation = $creationTamperValidation.report
        stderr = $creationTamperValidation.stderr
    }
})

$preflightSnapshotTamperPath = Copy-Case `
    -Name 'preflight-absence-snapshot-tamper'
$preflightSnapshotPath = Join-Path $preflightSnapshotTamperPath 'preflight.json'
$preflightSnapshot = Get-Content -Raw -LiteralPath $preflightSnapshotPath |
    ConvertFrom-Json
$preflightSnapshot.listener_records = @([ordered]@{
    LocalAddress = '127.0.0.1'
    LocalPort = [int]$plan.port
    State = 'Listen'
    OwningProcess = 99999
})
Write-JsonLf -Path $preflightSnapshotPath -Value $preflightSnapshot
Update-CaseManifest -Path $preflightSnapshotTamperPath
Add-RejectionResult -Results $results `
    -Name 'preflight_absence_claim_requires_empty_snapshots' `
    -Path $preflightSnapshotTamperPath
...
$foreignVerifierPath =
    "$env:SystemRoot\System32;$env:SystemRoot\System32\Wbem;" +
    "$env:SystemRoot;$gitDirectory"
if ($foreignVerifierPath -ceq $parentPath) {
    $foreignVerifierPath += ';C:\AF-S114-E03-VERIFIER-PATH'
}
try {
...
$results.Add([ordered]@{
    ...
    evidence = [ordered]@{
        ...
        verifier_git_path_sha256 = Get-Sha256Text -Text $gitCommand.Source
...
$parentPathTamperLaunch.declared_parent_environment.Path =
    "$parentPath;C:\AF-S114-E03-TAMPER"
...
$epochTwoRawRoot = Join-Path $repoRoot `
    'target/s114-experiment/runtime-epoch-2/smoke/e02-01-q4-32768'
$epochOneArtifactDir = Split-Path -Parent $artifactDir
$epochTwoArtifactDir = Join-Path $epochOneArtifactDir 'epoch-2'
$epochOneArchive = Join-Path $epochOneArtifactDir `
    'attempts/01-q4-32768'
$epochTwoArchive = Join-Path $epochTwoArtifactDir `
    'attempts/e02-01-q4-32768'
$epochThreeArchive = Join-Path $artifactDir "attempts/$coordinate"
$normalizedRawRoots = @(
    @($epochOneRawRoot, $epochTwoRawRoot, $canonicalRawRoot) |
        ForEach-Object {
            [System.IO.Path]::GetFullPath($_).ToLowerInvariant()
        }
)
$normalizedArchives = @(
    @($epochOneArchive, $epochTwoArchive, $epochThreeArchive) |
        ForEach-Object {
            [System.IO.Path]::GetFullPath($_).ToLowerInvariant()
        }
)
$results.Add([ordered]@{
    ...
    passed = (
        $coordinate -match '^e03-' -and
        @($normalizedRawRoots | Select-Object -Unique).Count -eq 3 -and
        @($normalizedArchives | Select-Object -Unique).Count -eq 3 -and
    ...
    evidence = [ordered]@{
        ...
        epoch_2_raw = $epochTwoRawRoot
        epoch_3_raw = $canonicalRawRoot
        epoch_1_archive = $epochOneArchive
        epoch_2_archive = $epochTwoArchive
        epoch_3_archive = $epochThreeArchive
    }
})

$coverageFixtureRoot = Join-Path $testRoot 'coverage-fixture/runtime'
$coverageFixtureEpochTwo = Join-Path $coverageFixtureRoot 'epoch-2'
$coverageFixtureEpochThree = Join-Path $coverageFixtureRoot 'epoch-3'
$coverageFixtureStage = Join-Path $coverageFixtureEpochThree `
    '.final-stage-fixture'
$coverageFixtureEpochOneAttempt = Join-Path $coverageFixtureRoot `
    'attempts/01-q4-32768'
$coverageFixtureEpochTwoAttempt = Join-Path $coverageFixtureEpochTwo `
    'attempts/e02-01-q4-32768'
$coverageFixtureEpochThreeAttempt = Join-Path $coverageFixtureEpochThree `
    'attempts/e03-01-q4-32768'
$coverageFixtureFinal = Join-Path $coverageFixtureEpochThree 'final'
foreach ($directory in @(
    $coverageFixtureStage,
    $coverageFixtureEpochOneAttempt,
    $coverageFixtureEpochTwoAttempt,
    $coverageFixtureEpochThreeAttempt,
...
Write-Utf8Lf -Path (Join-Path $coverageFixtureRoot 'runtime-plan.json') `
    -Text "{}`n"
Write-Utf8Lf -Path (Join-Path $coverageFixtureEpochOneAttempt 'attempt.json') `
    -Text "{}`n"
Write-Utf8Lf -Path (Join-Path $coverageFixtureEpochTwo 'runtime-plan.json') `
    -Text "{}`n"
Write-Utf8Lf -Path (Join-Path $coverageFixtureEpochTwoAttempt 'attempt.json') `
    -Text "{}`n"
Write-Utf8Lf -Path (Join-Path $coverageFixtureEpochThree 'runtime-plan.json') `
    -Text "{}`n"
Write-Utf8Lf -Path (Join-Path $coverageFixtureEpochThreeAttempt 'attempt.json') `
    -Text "{}`n"
...
$prepublicationCoverage = @(Get-RuntimeCoverageRecords `
    ...
    -EpochArtifactDirectory $coverageFixtureEpochThree `
    -StageDirectory $coverageFixtureStage)

$publishedCoverageRoot = Join-Path $testRoot 'coverage-published/runtime'
$publishedCoverageEpochTwo = Join-Path $publishedCoverageRoot 'epoch-2'
$publishedCoverageEpochThree = Join-Path $publishedCoverageRoot 'epoch-3'
$publishedCoverageEpochOneAttempt = Join-Path $publishedCoverageRoot `
    'attempts/01-q4-32768'
$publishedCoverageEpochTwoAttempt = Join-Path $publishedCoverageEpochTwo `
    'attempts/e02-01-q4-32768'
$publishedCoverageEpochThreeAttempt = Join-Path $publishedCoverageEpochThree `
    'attempts/e03-01-q4-32768'
$publishedCoverageFinal = Join-Path $publishedCoverageEpochThree 'final'
foreach ($directory in @(
    $publishedCoverageEpochOneAttempt,
    $publishedCoverageEpochTwoAttempt,
    $publishedCoverageEpochThreeAttempt,
...
Copy-Item -LiteralPath (Join-Path $coverageFixtureRoot 'runtime-plan.json') `
    -Destination (Join-Path $publishedCoverageRoot 'runtime-plan.json')
Copy-Item -LiteralPath (Join-Path $coverageFixtureEpochOneAttempt `
    'attempt.json') -Destination (Join-Path $publishedCoverageEpochOneAttempt `
    'attempt.json')
Copy-Item -LiteralPath (Join-Path $coverageFixtureEpochTwo `
    'runtime-plan.json') -Destination (Join-Path $publishedCoverageEpochTwo `
    'runtime-plan.json')
Copy-Item -LiteralPath (Join-Path $coverageFixtureEpochTwoAttempt `
    'attempt.json') -Destination (Join-Path $publishedCoverageEpochTwoAttempt `
    'attempt.json')
Copy-Item -LiteralPath (Join-Path $coverageFixtureEpochThree `
    'runtime-plan.json') -Destination (Join-Path $publishedCoverageEpochThree `
    'runtime-plan.json')
Copy-Item -LiteralPath (Join-Path $coverageFixtureEpochThreeAttempt `
    'attempt.json') -Destination (Join-Path $publishedCoverageEpochThreeAttempt `
    'attempt.json')
...
$publishedCoverage = @(Get-RuntimeCoverageRecords `
    ...
    -EpochArtifactDirectory $publishedCoverageEpochThree)
$expectedCoveragePaths = @(
    'runtime-plan.json',
    'attempts/.gitattributes',
    'attempts/01-q4-32768/attempt.json',
    'epoch-2/attempts/e02-01-q4-32768/attempt.json',
    'epoch-2/runtime-plan.json',
    'epoch-3/attempts/e03-01-q4-32768/attempt.json',
    'epoch-3/final/runtime-verification.json',
    'epoch-3/final/selection.json',
    'epoch-3/runtime-plan.json'
) | Sort-Object
...
$results.Add([ordered]@{
    name = 'final_manifest_covers_all_runtime_epochs'
    passed = (
...
$baselineRecovery = Test-RecoveryAnchors -Plan $plan -RepositoryRoot $repoRoot
if (-not $baselineRecovery.passed) {
    throw "baseline prior-epoch recovery anchors are invalid: $($baselineRecovery.errors -join '; ')"
}

$badAnchorPlan = $plan | ConvertTo-Json -Depth 100 | ConvertFrom-Json
$badAnchorEpoch = @($badAnchorPlan.recovery.prior_epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$badAnchor = @($badAnchorEpoch[0].anchors | Where-Object {
    $_.role -ceq 'text_policy'
})
$badAnchor[0].sha256 = '0' * 64
$badAnchorResult = Test-RecoveryAnchors -Plan $badAnchorPlan `
    -RepositoryRoot $repoRoot
$badAnchorEpochResult = @($badAnchorResult.epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$results.Add([ordered]@{
    name = 'prior_epoch_anchor_mismatch_rejected'
    passed = (
        $badAnchorEpoch.Count -eq 1 -and
        $badAnchor.Count -eq 1 -and
        -not $badAnchorResult.passed -and
        $badAnchorEpochResult.Count -eq 1 -and
        -not $badAnchorEpochResult[0].passed -and
        @($badAnchorResult.errors | Where-Object {
            $_ -match '^epoch-2: recovery anchor differs:'
        }).Count -eq 1
    )
    evidence = $badAnchorResult
})

$badDigestPlan = $plan | ConvertTo-Json -Depth 100 | ConvertFrom-Json
$badDigestEpoch = @($badDigestPlan.recovery.prior_epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$badDigestAnchor = @($badDigestEpoch[0].anchors | Where-Object {
    $_.role -ceq 'control_manifest'
})
$badDigestAnchor[0].sha256 = '0' * 64
$badDigestResult = Test-RecoveryAnchors -Plan $badDigestPlan `
    -RepositoryRoot $repoRoot
$badDigestEpochResult = @($badDigestResult.epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$results.Add([ordered]@{
    name = 'prior_epoch_control_digest_mismatch_rejected'
    passed = (
        $badDigestEpoch.Count -eq 1 -and
        $badDigestAnchor.Count -eq 1 -and
        -not $badDigestResult.passed -and
        $badDigestEpochResult.Count -eq 1 -and
        -not $badDigestEpochResult[0].control_digest_passed
    )
    evidence = $badDigestResult
})

$attemptTreeFixture = Join-Path $testRoot 'prior-attempt-tree-mismatch'
[System.IO.Directory]::CreateDirectory($attemptTreeFixture) | Out-Null
$attemptTreeManifest = Join-Path $attemptTreeFixture 'files.sha256'
Write-Utf8Lf -Path $attemptTreeManifest `
    -Text "$('0' * 64)  missing-evidence.json`n"
$badAttemptTreePlan = $plan | ConvertTo-Json -Depth 100 | ConvertFrom-Json
$badAttemptTreeEpoch = @(
    $badAttemptTreePlan.recovery.prior_epochs | Where-Object {
        [int]$_.epoch -eq 2
    }
)
$badAttemptTreeAnchor = @($badAttemptTreeEpoch[0].anchors | Where-Object {
    $_.role -ceq 'attempt_manifest'
})
$badAttemptTreeAnchor[0].relative_path = Get-RelativeSlashPath `
    -Root $repoRoot -Path $attemptTreeManifest
$badAttemptTreeAnchor[0].bytes = [UInt64](
    Get-Item -LiteralPath $attemptTreeManifest
).Length
$badAttemptTreeAnchor[0].sha256 = Get-Sha256Lower -Path $attemptTreeManifest
$badAttemptTreeResult = Test-RecoveryAnchors -Plan $badAttemptTreePlan `
    -RepositoryRoot $repoRoot
$badAttemptTreeEpochResult = @($badAttemptTreeResult.epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$results.Add([ordered]@{
    name = 'prior_epoch_attempt_tree_mismatch_rejected'
    passed = (
        $badAttemptTreeEpoch.Count -eq 1 -and
        $badAttemptTreeAnchor.Count -eq 1 -and
        -not $badAttemptTreeResult.passed -and
        $badAttemptTreeEpochResult.Count -eq 1 -and
        -not $badAttemptTreeEpochResult[0].attempt_manifest_passed
    )
    evidence = $badAttemptTreeResult
})

$gitProtectionFixture = Join-Path $testRoot 'prior-git-protection-mismatch'
[System.IO.Directory]::CreateDirectory($gitProtectionFixture) | Out-Null
$epochTwoAttemptPath = Join-Path $epochTwoArchive 'attempt.json'
$unprotectedAttemptPath = Join-Path $gitProtectionFixture 'attempt.json'
Copy-Item -LiteralPath $epochTwoAttemptPath -Destination $unprotectedAttemptPath
$badProtectionPlan = $plan | ConvertTo-Json -Depth 100 | ConvertFrom-Json
$badProtectionEpoch = @($badProtectionPlan.recovery.prior_epochs |
    Where-Object { [int]$_.epoch -eq 2 })
$badProtectionAnchor = @($badProtectionEpoch[0].anchors | Where-Object {
    $_.role -ceq 'attempt'
})
$badProtectionAnchor[0].relative_path = Get-RelativeSlashPath `
    -Root $repoRoot -Path $unprotectedAttemptPath
$badProtectionAnchor[0].bytes = [UInt64](
    Get-Item -LiteralPath $unprotectedAttemptPath
).Length
$badProtectionAnchor[0].sha256 = Get-Sha256Lower -Path $unprotectedAttemptPath
$badProtectionResult = Test-RecoveryAnchors -Plan $badProtectionPlan `
    -RepositoryRoot $repoRoot
$badProtectionEpochResult = @($badProtectionResult.epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$results.Add([ordered]@{
    name = 'prior_epoch_git_protection_mismatch_rejected'
    passed = (
        $badProtectionEpoch.Count -eq 1 -and
        $badProtectionAnchor.Count -eq 1 -and
        -not $badProtectionResult.passed -and
        $badProtectionEpochResult.Count -eq 1 -and
        $badProtectionEpochResult[0].attempt_identity_passed -and
        -not $badProtectionEpochResult[0].git_protection_passed
    )
    evidence = $badProtectionResult
})

$badTerminalPlan = $plan | ConvertTo-Json -Depth 100 | ConvertFrom-Json
$badTerminalEpoch = @($badTerminalPlan.recovery.prior_epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$badTerminalEpoch[0].expected.verdict = 'viable'
$badTerminalResult = Test-RecoveryAnchors -Plan $badTerminalPlan `
    -RepositoryRoot $repoRoot
$badTerminalEpochResult = @($badTerminalResult.epochs | Where-Object {
    [int]$_.epoch -eq 2
})
$results.Add([ordered]@{
    name = 'epoch_2_terminal_identity_tamper_rejected'
    passed = (
        $badTerminalEpoch.Count -eq 1 -and
        -not $badTerminalResult.passed -and
        $badTerminalEpochResult.Count -eq 1 -and
        -not $badTerminalEpochResult[0].attempt_identity_passed
    )
    evidence = $badTerminalResult
})

$badMeasurementPlan = $plan | ConvertTo-Json -Depth 100 | ConvertFrom-Json
$badMeasurementPlan.port = [int]$badMeasurementPlan.port + 1
$badMeasurementResult = Test-EpochThreeMeasurementContinuity `
    -Plan $badMeasurementPlan -RepositoryRoot $repoRoot
$badPortField = @($badMeasurementResult.fields | Where-Object {
    $_.field -ceq 'port'
})
$results.Add([ordered]@{
    name = 'measurement_contract_drift_rejected'
    passed = (
        -not $badMeasurementResult.passed -and
        $badPortField.Count -eq 1 -and
        -not $badPortField[0].passed -and
        @($badMeasurementResult.errors | Where-Object {
            $_ -ceq 'measurement contract drifted: port'
        }).Count -eq 1
    )
    evidence = $badMeasurementResult
})
...
$manifestEscapeParent = Join-Path $testRoot 'manifest-path-escape'
$manifestEscapeRoot = Join-Path $manifestEscapeParent 'root'
[System.IO.Directory]::CreateDirectory($manifestEscapeRoot) | Out-Null
$manifestEscapeOutside = Join-Path $manifestEscapeParent 'outside.txt'
Write-Utf8Lf -Path $manifestEscapeOutside -Text "outside`n"
$manifestEscapePath = Join-Path $manifestEscapeRoot 'files.sha256'
Write-Utf8Lf -Path $manifestEscapePath -Text (
    "$(Get-Sha256Lower -Path $manifestEscapeOutside)  ../outside.txt`n"
)
$manifestEscapeResult = Test-HashManifest -Root $manifestEscapeRoot `
    -ManifestPath $manifestEscapePath -RejectUnlistedFiles
$results.Add([ordered]@{
    name = 'manifest_path_escape_rejected'
    passed = (
        -not $manifestEscapeResult.passed -and
        @($manifestEscapeResult.errors | Where-Object {
            $_ -ceq 'unsafe manifest path: ../outside.txt'
        }).Count -eq 1
    )
    evidence = $manifestEscapeResult
})
...
try {
    $contender = Invoke-PowerShellFileBounded -ScriptPath $runnerPath `
        -Arguments @('-Coordinate', 'e03-01-q4-32768') `
        -TimeoutMilliseconds 30000
}
finally {
    $lockProbe.Dispose()
...
$report = [ordered]@{
    schema = 'animus-ferric-runtime-self-test-v3'
    task = 'T-11409'
    control_epoch = 3
    attestation_protocol = [string]$plan.template_attestation.protocol
    process_command_protocol =
