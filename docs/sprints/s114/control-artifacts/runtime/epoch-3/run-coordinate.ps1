...
if (-not (Test-RuntimePlanIdentity -Plan $plan)) {
    throw 'runtime plan is not the frozen epoch-3 recovery protocol'
}
...
function Assert-ControlInputs {
    ...
    if ($controlManifest.schema -cne
        ...
        -not (Test-JsonEquivalent -Left @($controlManifest.prior_epochs) `
        ...
        throw 'runtime control manifest is not the frozen epoch-3 recovery'
    }
    ...
    if (($observedControlNames -join "`n") -cne
        ...
        throw 'runtime control manifest does not name the exact control set'
    }
    ...
    if (-not (Test-JsonEquivalent -Left $recoveryAnchors `
            -Right $controlManifest.recovery_anchors)) {
        throw 'live prior-epoch anchors differ from their frozen result'
    }
    $measurementContinuity = Test-EpochThreeMeasurementContinuity `
        -Plan $plan -RepositoryRoot $repoRoot
    if (-not $measurementContinuity.passed -or
        -not (Test-JsonEquivalent -Left $measurementContinuity `
            -Right $controlManifest.measurement_continuity)) {
        throw 'epoch-3 measurement contract differs from the frozen continuity proof'
    }
    ...
    foreach ($control in $controlManifest.controls) {
        ...
        if ($relative -notmatch '^[A-Za-z0-9._-]+$' -or
            ...
            throw "unsafe or duplicate runtime control path: $relative"
        }
        $path = Resolve-SafeRelativePath -Root $artifactDir `
            -RelativePath $relative
        if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or
            [UInt64](Get-Item -LiteralPath $path).Length -ne
                [UInt64]$control.bytes -or
            (Get-Sha256Lower -Path $path) -cne [string]$control.sha256) {
            throw "runtime control changed after freeze: $relative"
        }
    }
...
function Assert-ColdCapacity {
    ...
    $script:preflightListenerRecords = @(
    ...
    $script:preflightLlamaProcessRecords = @($llamaProcesses)
...
function Assert-CoordinateAuthorization {
    if ($null -ne $coordinatePlan.predecessor) {
        ...
        if (-not $manifestCheck.passed -or
            ...
            $predecessor.schema -ne 'animus-ferric-runtime-attempt-v3' -or
            [int]$predecessor.control_epoch -ne 3 -or
    ...
    if ($coordinatePlan.quant -eq 'Q3_K_XL') {
        ...
        if ($gateVerificationCode -ne 0 -or
            $gateVerification.schema -cne
                'animus-ferric-q4-gate-verification-v3' -or
            $gateVerification.task -cne 'T-11409' -or
            [int]$gateVerification.control_epoch -ne 3 -or
            $gateVerification.attestation_protocol -cne
                [string]$plan.template_attestation.protocol -or
            $gateVerification.process_command_protocol -cne
                [string]$plan.process_command_attestation.protocol -or
...
try {
    ...
    if (([string]$repositoryCommit).Trim() -cne
        ...
        throw 'repository HEAD changed after the epoch-3 runtime freeze'
    }
    ...
    $preflight = [ordered]@{
        ...
        listener_records = @($preflightListenerRecords)
        llama_server_process_records = @($preflightLlamaProcessRecords)
        listener_absent = @($preflightListenerRecords).Count -eq 0
        any_llama_server_process_absent =
            @($preflightLlamaProcessRecords).Count -eq 0
    ...
    if ($startupHealthy) {
        ...
        $processCommandBinding = if ($null -ne $process) {
            Test-BoundWindowsProcessCommand `
                -ExecutablePath ([string]$process.ExecutablePath) `
                -ExecutableSha256 $processHash `
                -CommandLine ([string]$process.CommandLine) `
                -FrozenExecutablePath $llamaPath `
                -FrozenExecutableSha256 `
                    ([string]$plan.llama_cpp.expected_server_sha256) `
                -ExpectedArgv @($launchDeclaration.expected_llama_argv)
        }
        else {
            $null
        }
        $attestationCapturedAt = (Get-Date).ToUniversalTime()
        $processCreationUtc = if ($null -ne $process) {
            try {
        ...
        $attestation = [ordered]@{
            ...
            coordinate = $Coordinate
            captured_at_utc = $attestationCapturedAt.ToString('o')
            runfiles = [ordered]@{
