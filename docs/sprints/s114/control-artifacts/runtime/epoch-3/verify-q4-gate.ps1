...
if (-not (Test-RuntimePlanIdentity -Plan $plan)) {
    $errors.Add('runtime plan is not the epoch-3 recovery plan')
}
...
function Get-VerifiedAttempt {
    ...
    if ($verificationCode -ne 0 -or
        $verification.schema -cne
            'animus-ferric-runtime-verification-v3' -or
        $verification.task -cne 'T-11409' -or
        [int]$verification.control_epoch -ne 3 -or
...
$primary = Get-VerifiedAttempt -Id 'e03-01-q4-32768'
if ($null -ne $primary) {
    $chain.Add($primary)
}
...
if ($requiresRetry) {
    $retry = Get-VerifiedAttempt -Id 'e03-02-q4-16384'
    if ($null -ne $retry) {
        $chain.Add($retry)
    }
}
...
if (-not $DeriveOnly) {
    ...
    if ($null -ne $gate) {
        if ($gate.schema -ne 'animus-ferric-qwen38-viability-v3' -or
            $gate.task -cne 'T-11409' -or
            [int]$gate.control_epoch -ne 3 -or
...
$report = [ordered]@{
    schema = 'animus-ferric-q4-gate-verification-v3'
    task = 'T-11409'
    control_epoch = 3
