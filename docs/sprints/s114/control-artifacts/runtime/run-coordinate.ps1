...
$rawParent = Join-Path $repoRoot 'target/s114-experiment/smoke'
...
function Assert-CoordinateAuthorization {
    if ($null -ne $coordinatePlan.predecessor) {
        ...
        if (-not $manifestCheck.passed -or
            ...
            $predecessor.schema -ne 'animus-ferric-runtime-attempt-v1' -or
...
try {
    ...
    if (@($deviceOutput | Where-Object { $_ -match 'CUDA0:.*RTX 2080 Ti' }).Count -ne 1) {
        throw 'the frozen CUDA runtime no longer exposes the declared GPU'
    }
    ...
    if ($startupHealthy) {
        ...
        $defaultTemplateKwargs = Get-OptionalProperty `
            -Value $propsJson -Name 'default_template_kwargs'
        ...
        $coreAttestationPassed =
            ...
            ($enableThinkingKwarg -eq $true) -and
        ...
        $attestation = [ordered]@{
            ...
            endpoints = [ordered]@{
                ...
                default_template_kwargs = $defaultTemplateKwargs
            }
            requested = [ordered]@{
            ...
            effective = [ordered]@{
                ...
                preserve_reasoning_enabled = $preserveReasoningEnabled
                preserve_reasoning_evidence_source =
                    'props.default_template_kwargs'
                reasoning_budget = [int]$plan.server.environment.LLAMA_ARG_THINK_BUDGET
