[CmdletBinding()]
param(
    [ValidateSet('03-q3-32768', '04-q3-16384')]
    [string]$Q3AttemptId
)
...
function Assert-FrozenControls {
    ...
    if ($controlManifest.schema -cne
            'animus-ferric-runtime-control-inputs-v1' -or
        $controlManifest.runtime_plan_sha256 -cne
            (Get-Sha256Lower -Path $planPath)) {
        throw 'frozen control manifest is malformed or names another runtime plan'
    }

    $seen = [System.Collections.Generic.HashSet[string]]::new(
        [System.StringComparer]::Ordinal
    )
    foreach ($entry in @($controlManifest.controls)) {
    ...
    foreach ($required in @(
        ...
        if (-not $seen.Contains($required)) {
            throw "frozen control manifest omits finalization authority: $required"
        }
    }
    $controlManifest
}
...
function Invoke-Q4GateVerification {
    ...
    if ([int]$processResult.exit_code -ne 0 -or
        $report.schema -cne 'animus-ferric-q4-gate-verification-v1' -or
...
function Get-VerifiedAttempt {
    ...
    if ([int]$processResult.exit_code -ne 0 -or
        $report.schema -cne 'animus-ferric-runtime-verification-v1' -or
        $report.coordinate -cne $Id -or
        -not [bool]$report.passed -or
        -not [bool]$report.manifest.passed) {
    ...
    if ($attempt.schema -cne 'animus-ferric-runtime-attempt-v1' -or
...
function Get-CoverageRecords {
    ...
    $stagePrefix = if ([string]::IsNullOrWhiteSpace($StageDirectory)) {
        ...
        [System.IO.Path]::GetFullPath($StageDirectory).TrimEnd(
            [System.IO.Path]::DirectorySeparatorChar,
            [System.IO.Path]::AltDirectorySeparatorChar
        ) + [System.IO.Path]::DirectorySeparatorChar
    }
    foreach ($file in @(Get-ChildItem -LiteralPath $artifactDir -Recurse `
        -File -Force -ErrorAction Stop)) {
        $relative = Get-RelativeSlashPath -Root $artifactDir -Path $file.FullName
...
if ($plan.schema -cne 'animus-ferric-runtime-plan-v1' -or
    $plan.task -cne 'T-11409') {
    throw 'runtime plan identity is malformed'
}
if (Test-Path -LiteralPath $finalDir) {
    throw 'runtime/final already exists and will not be overwritten'
}
...
if ($q4Ids.Count -lt 1 -or $q4Ids.Count -gt 2 -or
    $q4Ids[0] -cne '01-q4-32768' -or
    ($q4Ids.Count -eq 2 -and $q4Ids[1] -cne '02-q4-16384') -or
...
elseif ($q4Derivation.q4_verdict -ceq 'non_viable') {
    if ([bool]$q4Derivation.q3_fallback_authorized) {
        $q3PrimaryProbe = Get-VerifiedAttempt -Id '03-q3-32768'
        $expectedIds.Add('03-q3-32768')
        $q3TerminalId = '03-q3-32768'
        if ($q3PrimaryProbe.attempt.failure_classification -ceq
                'startup_memory_pressure') {
            Get-VerifiedAttempt -Id '04-q3-16384' | Out-Null
            $expectedIds.Add('04-q3-16384')
            $q3TerminalId = '04-q3-16384'
        }
...
$selection = [ordered]@{
    schema = 'animus-ferric-qwen38-selection-v1'
    task = 'T-11409'
    result = $finalLabel
...
$verification = [ordered]@{
    schema = 'animus-ferric-runtime-tests-v1'
...
try {
    ...
    $manifest = [ordered]@{
        schema = 'animus-ferric-runtime-artifact-manifest-v1'
        task = 'T-11409'
