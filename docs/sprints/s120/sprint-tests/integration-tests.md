...
**Final implementation HEAD: pending. Final native/CI results: pending. Formal
Test acceptance: pending.** This is an assertion inventory and invocation map,
...
|---|---|---|
| W-WIN | `cargo test --workspace --locked` on native Windows | Pending final HEAD/result |
| W-LINUX | `bash tools/test-lifecycle-linux.sh workspace` in Linux CI | Pending final HEAD/native CI result |
| N | `cargo test -p ferric-cli --no-default-features --locked` | Pending final HEAD/results on Windows and Linux |
| H | `cargo test --locked -p ferric-cli --test human_cli --test human_docs --test source_execution --test template_hygiene` | Pending final HEAD/result |
| HU | `cargo test --locked -p ferric-cli --bin ferric human:: -- --test-threads=1` on native Windows | Pending final HEAD/result; live test remains ignored here |
| S | `cargo test --locked -p ferric-cli --features backend-openai --bin ferric startup:: -- --test-threads=1` on native Windows | Pending final HEAD/result |
| P | `cargo test --locked -p ferric-provider --features backend-openai --lib` | Pending final HEAD/result |
| PY | `cargo test --locked -p ferric-tools --lib check_syntax` | Pending final HEAD/result |
| M | `cargo test --locked -p ferric-tools --test controlled_mutations` | Pending final HEAD/result |
| CLI | `cargo test --locked -p ferric-cli --test cli` | Pending final HEAD/result |
| L | `cargo test --locked -p ferric-cli --bin ferric real_model_prepared_host_journey -- --ignored --exact human::enabled::tests::real_model_prepared_host_journey --nocapture --test-threads=1` | Pending final HEAD/live result |
| TTY | Actual terminal `cargo r -- run --workspace "<fresh-temporary-workspace>" --model "<existing-local-model.gguf>"`, then the bounded Ask transcript | Pending final HEAD/transcript; earlier Build transcript is in E2E evidence |
| F | `cargo fmt --all --check` | Pending final HEAD/result |
| FI | `rustfmt --edition 2024 --check crates/ferric-cli/src/human_journey_tests.rs` | Pending final HEAD/result; include!-based fixture formatting is checked explicitly |
| C | `cargo clippy --workspace --all-targets --locked -- -D warnings` | Pending final HEAD/result |
| CN | `cargo clippy -p ferric-cli --no-default-features --all-targets --locked -- -D warnings` | Pending final HEAD/result |
| LF-WIN | `cargo test -p ferric-cli --features lifecycle-fixture --test server_lifecycle_fixture --locked -- --test-threads=1` | Pending final HEAD/result |
...
| E02-A consumer admission | `crates/ferric-cli/tests/cli.rs::present_invalid_config_blocks_all_consumers`, `invalid_effective_numbers_rejected_by_cli_surfaces`; `crates/ferric-cli/src/api.rs::present_invalid_config_blocks_all_consumers_api_reload`; `crates/ferric-cli/tests/human_cli.rs::human_invalid_config_blocks_before_preparation` | Legacy query/chat/MCP/skills/API malformed/enum/numeric/directory matrix, ICM invalid admission, effective-number benchmark checks, API HTTP error classification, and new run/status/explain rejection before lock/state preparation. Diagnostic credentials are excluded and prior config bytes retained. The human assertion is a new addition whose final execution result is pending. CLI, H, W-WIN, W-LINUX, applicable N cases. |
...
validity, not a new product capability. Their final execution results are pending.
