...
## T-3905 (sprint 39)
- **Description:** **`ferric query --resume <path>` CLI wiring.** `QueryArgs.prompt` becomes `Option<String>` with clap `required_unless_present = "resume"` (byte-identical usage-error behavior when `--resume` is never used); new `QueryArgs.resume: Option<PathBuf>`. `run_query` calls `ferric_loop::replay(path)` right after `build_run_config` (needs `config.protocol` for the match check): on `AlreadyStopped`/`MissingSessionPrompt`/io error, prints a clear message naming the path and exits `FAILURE` without running; on a protocol mismatch between the replayed trace and this invocation's resolved protocol, prints both names and exits `FAILURE`. `effective_prompt` becomes `Option<String>` (`None` for a pure continuation with no `--file`s either). **C-009 (plan-critic)**: when `--resume` is combined with a resolved `--prompts-dir`/an `Animus.md`-bearing workspace, prints a stderr note that both are inert for the resumed run's (frozen) system message — checked via `config.lineage`/`config.prompt_composition_error` (composition actually ATTEMPTED), not just whether `--prompts-dir` was passed, since `FERRIC_PROMPTS_DIR` can also resolve it. 6 new CLI subprocess tests (`crates/ferric-cli/tests/cli.rs`) against a hand-written, `ferric-loop::replay`-shaped trace fixture (`SessionPrompt` + `PolicySelected` + one completed turn, no `SessionEnd` — the interrupted-process shape) run through the REAL binary: a plain resume, resume + extra prompt, the no-resume-no-prompt regression, the protocol-mismatch error, the already-stopped error, and the Animus.md-ignored note. `cargo test --workspace` green (including `--features backend-openai`); clippy/fmt clean.
- **Completed:** 2026-07-04 (build phase)
- **Files modified:** crates/ferric-cli/src/query.rs, crates/ferric-cli/tests/cli.rs
- **Commit:** `e85bf37`
...
## T-4001 (sprint 40)
- **Description:** **New trace variant `Event::HistoryCompacted { through_turn, dropped_turns, summary }` — purely additive, no `#[serde(default...)]` needed (a brand-new variant, not an extension of an existing one).** `crates/ferric-trace/src/event.rs` gains the variant; `all_event_types()`/`jsonl_roundtrip_all_event_types` extended plus a dedicated `history_compacted_roundtrip` test. **Compiler-coupled scope note:** `trace_cmd.rs`'s `render()` and `common/mod.rs`'s `kinds()` test helper both match exhaustively over `Event`, so Rust's exhaustiveness check forces both to be updated the moment the enum grows a variant — the identical coupling sprint 39's T-3901 hit (it folded `trace_cmd.rs` into the event-adding commit for the same reason). T-4005's CLI legibility work therefore ships in this same commit: `render()` now shows the folded-turn count and a legible summary excerpt, verified by a new `cli::trace_cat_renders_history_compacted` test. `cargo test --workspace` green; clippy `-D warnings` clean; fmt clean.
- **Completed:** 2026-07-04 (build phase)
- **Files modified:** crates/ferric-trace/src/{event.rs,lib.rs}, crates/ferric-cli/src/trace_cmd.rs, crates/ferric-cli/tests/cli.rs, crates/ferric-loop/tests/common/mod.rs
- **Commit:** `35cd7cd`

## T-4002+T-4003 (sprint 40)
- **Description:** **`HistoryCompactor` (new `crates/ferric-loop/src/compact.rs`) + wired into `run.rs`'s turn loop.** Tracks `Vec<(u32 absolute_turn_number, usize start_index_in_messages)>` since the last fold — a design correction made during the plan-critic pass: an earlier draft planned a relative `turn_offset` accumulator whose role and update formula were never derived, only asserted; the critic also found `replay.rs`'s real `TurnStart{ .. }` match arm discards the turn number entirely, meaning the originally-planned extension needed real new plumbing, not a drop-in addition. Absolute-number tracking removes both problems at once: `through_turn` is read directly off the tracked pair (no arithmetic), the fold-span boundary is a closed form (`fold_to_idx = completed[fold_count].1` by construction of the slice split), and a resumed session's compactor (which may start counting from a nonzero `turns`) needs zero special-casing. `maybe_compact()` fires at 85% of `policy.prompt_budget_tokens` (fixed constant, matching a `deepagents`-style precedent from research) with `KEEP_LAST_TURNS = 2` always preserved verbatim (fixed constant, mirrors the Microsoft Agent Framework's `MinimumPreserved` floor, one of two external references reviewed in research); on trigger it summarizes the foldable span via a single-shot, no-tools completion reusing the SAME provider (no second, cheaper model exists in this architecture) through the existing `complete_with_backoff`; a summarizer failure (provider error or empty output) is non-fatal — logs one `Event::Note`, leaves `messages` unchanged. Wired into `run.rs`: `record_turn_start()`+`maybe_compact()` run right after `Event::TurnStart` is written each iteration — **this ordering is load-bearing** (structurally enforced: the compactor's `completed` slice excludes the just-pushed current-turn entry by construction, not just by call-order convention) and is what lets `HistoryCompacted` always land in the trace AFTER `TurnStart(current)`, which `replay()`'s pre-existing "commit on next TurnStart" rule (ADR-049) depends on. **Compiler-coupled scope note:** `HistoryCompactor` is deliberately `pub(crate)` (no public API surface, per the plan) — this trips dead-code analysis under `-D warnings` until something in-crate calls it, so T-4002 and T-4003 are not independently committable under this project's own lint gate and ship together. 6 new unit tests co-located in `compact.rs` (`render_transcript` role/tool-call rendering; below-trigger/not-enough-history no-ops; a successful fold's exact spliced shape + traced event fields; non-fatal summarizer-failure skip; the repeat-fold-never-accumulates case) plus 4 new integration tests in `crates/ferric-loop/tests/compaction_tests.rs` (a real triggered fold shrinking a later turn's `PromptAssembled.message_count` AND `.chars` — the actual token-budget-relevant metric per test-critic C-010's sprint-39 precedent; the direct trace byte-order regression for the load-bearing ordering invariant; the resumed-session-only-folds-new-turns scope limit; the resumed-session absolute-numbering-consistency proof). `cargo test --workspace` green; clippy `-D warnings` clean (default + `backend-openai` + `backend-mistralrs`); fmt clean.
- **Completed:** 2026-07-04 (build phase)
- **Files modified:** crates/ferric-loop/src/{compact.rs (new),lib.rs,run.rs}, crates/ferric-loop/tests/compaction_tests.rs (new)
- **Commit:** `81ce193`

## T-4004 (sprint 40)
- **Description:** **Extend `replay()` for `Event::HistoryCompacted` — the required cross-cutting fix, not an optional follow-on.** Sprint 39's `replay()` reconstructed `messages` purely from per-turn trace events, with no concept of a mid-session history rewrite — without this task, `--resume` of a compacted-then-killed session would resurrect the FULL pre-compaction history, silently defeating this sprint's entire purpose for exactly the long-running-session case that motivated it. `PendingTurn` gains a `turn: u32` field, captured from the `TurnStart{turn}` event that opens it (the real pre-sprint-40 code discarded this field entirely via `TurnStart{ .. }` — a plan-critic finding, C-001). `commit_and_reset!()` now pushes `(turn_num, messages.len())` into a new `committed_turn_starts: Vec<(u32, usize)>` before extending `messages`. On `Event::HistoryCompacted{through_turn, summary, ..}`: `partition_point` splits committed turns at `through_turn`, `messages` truncates back to `head_len` (tracked since the `SessionPrompt` arm), the summary is inserted, and the preserved tail is re-appended at shifted (never re-keyed — absolute turn numbers are unchanged) indices. Safe by construction per T-4003's ordering invariant: every turn `<= through_turn` is already committed by the time `HistoryCompacted` is reached, since it always follows the triggering turn's own `TurnStart`. Handles repeated compactions naturally — a second fold just re-partitions whatever survives the first. 3 new tests: `replay_applies_one_history_compaction` and `replay_applies_two_sequential_history_compactions` (co-located `#[cfg(test)]`, hand-authored synthetic traces matching `replay.rs`'s existing idiom — depend only on T-4001's event shape, not T-4003's `run.rs` wiring, per plan-critic C-005) plus `real_run_compact_kill_replay_resume_shrinks_history` (`compaction_tests.rs`): a REAL `run()` call triggers a real compaction, its real trace file is truncated (drop the trailing `session_end`, mirroring sprint 39's C-010 technique exactly), `replay()`d, and the resulting history is asserted to be EXACTLY the expected shrunk size (head + 1 summary + the preserved tail, folded turns and the dangling final turn both correctly absent) — then a second real `run()` resumes it and reaches `TaskComplete`. `cargo test --workspace` green; clippy `-D warnings` clean; fmt clean.
- **Completed:** 2026-07-04 (build phase)
- **Files modified:** crates/ferric-loop/src/replay.rs, crates/ferric-loop/tests/compaction_tests.rs
- **Commit:** `b5c5964`

## T-4006 (sprint 40)
- **Description:** **ADR-050 + docs.** Recorded the always-on/no-CLI-flag design and its guard-family precedent (ADR-037/038); the trigger fraction (85%) and `KEEP_LAST_TURNS` (2) as deliberate fixed constants for v1, not per-tier `RunPolicy` fields; the absolute-turn-number design correction made during plan critique (eliminating an originally-planned `turn_offset` re-keying scheme) and why it resolved three separate plan-critic concerns at once; the required (not optional) `replay()` extension and the ordering invariant it depends on; the same-provider-reuse architectural divergence from every framework surveyed in research; the non-fatal summarizer-failure decision (including the accepted ~1.75s worst-case backoff latency); the resume-interaction v1 scope limit (the entire replayed history is the compactor's protected head); and explicit deferrals (per-tier tuning, chunked summarization for a pathologically large first fold, a hard truncation backstop as a last-resort emergency fallback behind summarization). README Status bumped to sprint 40 + a new Sprint 40 timeline entry (replacing the stale "Next" line); the Sprint 40 backlog section rewritten from its in-progress checklist to a completed summary, explicitly naming both compiler-forced task merges (T-4001+T-4005, T-4002+T-4003) and why each was inseparable under this project's own lint gate rather than silently folding them in without comment.
- **Completed:** 2026-07-04 (build phase)
- **Files modified:** decisions.md, README.md, agent-tasks/agent-tasks.md, agent-tasks/completed-tasks.md
- **Commit:** `92960f0`

## T-4101 (sprint 41)
- **Description:** **ADR-051 — container architecture (sibling containers + microVM airlock, not DinD).** Records the core correction found in research: literal Docker-in-Docker (nested `dockerd`/mounted `docker.sock`) is a security anti-pattern for isolating untrusted content specifically (shared-kernel boundary; three 2024–2026 container→host CVEs cited), and 2026 practice for that use case has moved to microVM-class isolation (Docker Sandboxes GA Jan 2026 / gVisor) — the user's cited 2023 DinD article addresses a different problem (CI building other images). Splits the two conflated problems: platform-wide deployment flexibility → sibling containers via Docker Compose (k8s-ready); untrusted-content isolation (Ornstein's airlock) → a microVM sandbox. Records the `ferric-core`-as-one-container recommendation (verified: `crates/ferric-cli/src/server.rs` binds `host: "127.0.0.1"`, ADR-005 loopback-only — splitting harness/backend across containers would break that without extra network-namespace plumbing) and the `--features backend-openai` requirement (`ferric-cli`'s `default = []`). Lists all deferrals with one-sentence rationale (Ornstein-as-own-binary, MCP containerization, chat mode → sprint 42, multi-arch, live container-run, CI gate) and notes the mid-sprint Docker install clearing the multi-sprint blocker.
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** decisions.md, agent-tasks/agent-tasks.md
- **Commit:** `ed2203e`

## T-4102 (sprint 41)
- **Description:** **`docker/Dockerfile` — the `ferric-core` image (+ root `.dockerignore`).** Multi-stage: a `rust:1.96-slim` build stage (matching `rust-toolchain.toml`) running `cargo build --release --features backend-openai -p ferric-cli` (plan-critic C-001 — without the feature the built `ferric` can't drive the co-located backend over the HTTP valve), and a `debian:bookworm-slim` runtime stage installing the prebuilt `llama-server` b9821 Linux-x64 release (verified against the real GitHub release API: the Linux asset is `.tar.gz` not `.zip` — corrected mid-build; it extracts to `llama-b9821/` with `llama-server` + all `ggml`/`llama` `.so` libs flat, so `PATH`+`LD_LIBRARY_PATH` point there) alongside the compiled `ferric`. Non-root `ferric` user, `/workspace` workdir, no `EXPOSE` (loopback-only preserved structurally, ADR-005/ADR-051). x86_64/Linux-only skeleton (plan-critic C-002; multi-arch via buildx `--platform` deferred). A root `.dockerignore` keeps `COPY . .` lean (excludes `target/`, `.git/`, `sprints/`, `.github/`, model blobs). Structural checks pass (2 stages, `COPY --from=build` resolves, `--features backend-openai` present, no `EXPOSE`).
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** docker/Dockerfile (new), .dockerignore (new)
- **Commit:** `a5bc0a6`

## T-4103 (sprint 41)
- **Description:** **`docker/docker-compose.yml` — the sibling-container topology skeleton.** One fully-real service (`ferric-core`, `build: {context: .., dockerfile: docker/Dockerfile}`, a `../workspace:/workspace` bind mount, `restart: unless-stopped`). Future inter-service reachability is a COMMENT only — no functional `ports:`/`networks:` publish directive (plan-critic C-006: a live `ports:` mapping would contradict the Dockerfile's deliberate no-`EXPOSE` loopback-only stance in the same sprint). `ornstein-search` and `chat` are clearly-commented `# STUB` service blocks (no resolvable `build:`/`image:`), each naming the future sprint/ADR that fleshes it out (Ornstein inc 4 / sprint 42 respectively). A top-level comment block explains the topology and cross-references ADR-051. Structural checks pass (YAML valid; `ferric-core` the only active service, its `dockerfile:` path resolving to the real file, no `ports:`; both stubs commented, not active); `docker compose config` validates live (EXIT 0, resolves to only `ferric-core`).
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** docker/docker-compose.yml (new)
- **Commit:** `6efc852`

## T-4104 (sprint 41)
- **Description:** **`docs/ornstein.md` correction + ADR-051 docs wrap-up.** `docs/ornstein.md`'s Ornstein-inc-4 line, which named "(bollard/gVisor)" without distinguishing a container-orchestration client library (bollard) from an isolation mechanism, is corrected to name the microVM-sandbox recommendation (Docker Sandboxes primary, gVisor Linux-native alternative — NOT Docker-in-Docker) with a cross-reference to ADR-051, plus a note that Docker is now installed (increment no longer blocked). `agent-tasks/agent-tasks.md`'s own independent "(bollard/gVisor)" backlog line + its stale "BLOCKED on a containerizer / install Docker Desktop" text corrected in the same pass (plan-critic C-005 — the two files no longer diverge). README Status bumped to sprint 41 + a new Sprint 41 timeline entry; the stale "Next — TBD" line replaced with a sprint-42-chat-mode preview naming the now-unblocked Ornstein web increment; the Sprint 41 backlog section rewritten from its in-progress checklist to a completed summary (matching sprints 38–40's precedent), explicitly noting chat mode's deferral to sprint 42.
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** docs/ornstein.md, README.md, agent-tasks/agent-tasks.md, agent-tasks/completed-tasks.md
- **Commit:** `af727d7`

## T-4105 (sprint 41)
- **Description:** **Live `docker build` + `docker compose config` validation — and the GLIBC bug it caught.** With Docker Desktop installed mid-sprint (`linux/x86_64` engine), the skeleton artifacts were validated for real, not just structurally. `docker compose config` validated the compose file (EXIT 0, resolves to only `ferric-core`, no ports). `docker build -f docker/Dockerfile .` compiled the whole workspace in release with `--features backend-openai` (confirmed: `reqwest`/`rustls`/`hyper-rustls` compiled) and produced `ferric-core:s41` (206MB). **The value of live validation over structural checks was proven concretely**: a green `docker build` is NOT enough — running `docker run --rm ferric-core:s41 --version` surfaced a real runtime bug the build alone could never catch: `ferric: /lib/x86_64-linux-gnu/libc.so.6: version 'GLIBC_2.39' not found`. Root cause: the build stage used `rust:1.96-slim` (Debian-trixie-based, GLIBC 2.39) while the runtime stage was `debian:bookworm-slim` (GLIBC 2.36) — a dynamic-link mismatch. Fixed by pinning the build stage to `rust:1.96-slim-bookworm` (GLIBC-aligned with the runtime); rebuild + re-run confirmed `ferric 0.1.0` runs, `ferric --help` works, `llama-server` (b9821) is present on PATH and reports its version, and the container runs non-root (uid 10001). All live checks green after the fix.
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** docker/Dockerfile (GLIBC fix)
- **Commit:** `c7f70ca`

## T-4201 (sprint 42)
- **Description:** **ADR-052 — the chat security boundary (the ADR-011-revision's required dedicated ADR).** Records the hybrid talk+escalate decision and the full boundary: **talk mode** is the harness's FIRST unconstrained-completion path, structurally safe (empty `tools` + `constraint: None` = the lawful ADR-010 "neither" case + output text-only + never dispatched — safety is structural, not a prompt); **escalation** is USER-initiated only (ADR-005 — a model-initiated escalation would consult the LLM on a security decision, forbidden; talk output is appended as an assistant message and printed, never re-fed through the command parser, so there's no path by which a model's talk output could trigger an action); escalated `/do` turns reuse the constrained `run_with_provider`/`ferric-guard`/guards/trace path unchanged; the talk path lives in the CLI chat module while `ferric-loop::run()` stays always-constrained ("the loop always owns decoding" remains literally true); launch-time-fixed containment (ADR-046); the trace shape (talk → a chat-session log `Note`; each `/do` → its own fresh agentic file, since `run()` emits a whole envelope per call and can't be nested — the `ferric mcp` precedent); `--mock` uses a fresh per-turn `MockProvider` (a REPL's turn count is stdin-driven); explicit deferrals (TUI, talk-mode streaming, a dedicated `ChatTurn` trace event, Animus-IDE wiring).
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** decisions.md, agent-tasks/agent-tasks.md
- **Commit:** `ff01818`

## T-4202 (sprint 42)
- **Description:** **`crates/ferric-cli/src/chat.rs` — the hybrid talk+escalate REPL + `Command::Chat` wired into `main.rs`.** `ChatArgs` mirrors `QueryArgs` minus `prompt`/`resume`/`files` (launch-time-fixed containment). `run_chat` reuses the `ferric query`/`ferric mcp` scaffolding (`config::load_layered` + `build_run_config`) and a `ChatBackend` enum mirroring `mcp.rs`'s `Executor`: **Real** holds ONE provider + `tokio::runtime::Runtime` for the session; **Mock** builds a FRESH `MockProvider` per turn (talk-shaped `[text_completion]` vs the agentic `mock_provider(protocol)` script) so a stdin-driven REPL can never exhaust a fixed script (plan-critic C-001), with the `cfg(not(any(backend...)))` "use --mock" fallback (C-003/C-004). Pure helpers `parse_chat_input` (Talk/Escalate/Help/Exit/Empty; an UNKNOWN `/command` is Talked, never silently executed) and `talk_request` (empty tools + `None` constraint — the structural-safety invariant). Talk turns → `provider.complete(talk_request(...))`, print, append to history, `Note` to the chat-session log; `/do` turns → open a FRESH per-escalation `JsonlSink`, build a `ReplayedState` seed from the running history (`escalation_seed`: `messages` = history, `turns: 0` fresh budget, `protocol` = config, `source_session` = chat id), call `run_with_provider(prompt: Some(text), resume: Some(seed))`, fold the outcome into history. 6 co-located unit tests incl. `talk_request_has_no_action_channel`. **Live smoke confirmed the security boundary:** a talk line "write_file to somewhere" produced zero `tool_call`/`permission_check` events while `/do` genuinely dispatched (write landed in the workspace, full agentic trace in its own file). Compiles clean in the default (backend-free) build; `cargo test -p ferric-cli` (70 unit + 28 CLI) green.
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** crates/ferric-cli/src/chat.rs (new), crates/ferric-cli/src/main.rs
- **Commit:** `0b5bce5`

## T-4203 (sprint 42)
- **Description:** **Chat CLI subprocess tests (stdin-piped `ferric chat --mock`).** A new stdin-piping test harness — the suite's first (`run_chat_mock`: `Stdio::piped()` + batch `write_all` + close stdin + `wait_with_output`); `/exit` is handled purely at the parse layer (no `complete()` call) and the mock is fresh-per-turn, so an off-by-one in piped lines can't hang the child (plan-critic C-005). 4 tests: `chat_talk_then_exit` (talk response printed; exactly one coherent chat-session log with `session_start`/`note`/`session_end`; no escalation file), `chat_do_escalates_to_agentic_loop` (a SEPARATE `chat-esc-*.jsonl` with `tool_call`/`session_end` + the mock's write lands in the workspace — proving `/do` drove the guarded loop), `chat_help_lists_commands`, and `chat_talk_turn_is_not_dispatched` (the black-box structural-safety proof: a talk line that LOOKS like a tool call opens no escalation trace, writes no `tool_call`/`tool_result`/`permission_check` anywhere, and never touches the workspace). `cargo test --workspace` green; clippy `-D warnings` clean (default + both backend feature sets); fmt clean.
- **Completed:** 2026-07-09 (build phase)
- **Files modified:** crates/ferric-cli/tests/cli.rs
- **Commit:** `b0e0b88`
...
- [x] Edit crates/ferric-loop/src/grammar.rs to add thought field
- [x] Edit crates/ferric-loop/src/run.rs to update DEFAULT_SYSTEM_PROMPT
- [x] Edit crates/ferric-tools/src/builtin/write_file.rs to check is_dir
...
## T-10501/2/3/4 (sprint 105)
- **Description:** Detached the repo from the machine that grew it, applying sprint 97's `docker/.env` lesson everywhere else it held. **Untracked** (`git rm --cached`, files stay on disk): `sprints/` — **139 files tracked while already listed in `.gitignore`**, because a gitignore rule never untracks what already is — plus `scratch/` and `benchmarks/results.jsonl`. **Machine identity out of Rust fixtures:** real tailnet IP, MagicDNS suffix, hostname and node id in `server.rs`; device names, tailnet IPs and the owner's account handle in `retriever.rs`; a `C:\Users\<name>` path in an `animus-launch` comment. Every fixture keeps its exact shape with documentation values, addresses staying inside Tailscale's real `100.64.0.0/10` CGNAT range; all assertions still pass, which is the evidence none were asserting identity. **Defaults nobody else could satisfy:** `run_benchmarks.ps1`'s hardcoded `D:\Models\gguf\…` became a required parameter; `.env.example` now teaches the shape. **The behavioural one:** `benchmarks/model_profiles.json` is read back by ADR-029 and lets `measured_level` override the params prior in both directions, so a tracked copy gave every fresh checkout a tier measured on other hardware at an unpinned quantization — now gitignored, with a `model_profiles.example.json` whose measurements are `null`. **New `template_hygiene.rs`** (2 tests) fails on shapes that can only be identity while not matching topics; its own self-test caught that it missed backslash-doubled paths inside Rust string literals — the likeliest leak site — and the repo scan was shown failing against a planted leak. Docs updated. `decisions.md`/`agent-tasks/`/history deliberately not scrubbed, with the consequence recorded as a decision for the user.
- **Completed:** 2026-07-26
- **Files modified:** .gitignore, benchmarks/model_profiles.example.json (new), crates/ferric-cli/tests/template_hygiene.rs (new), crates/ferric-cli/src/server.rs, crates/ferric-research/src/retriever.rs, crates/animus-launch/src/lib.rs, tools/run_benchmarks.ps1, docker/.env.example, docs/{ornstein.md,beast-zoo-spec.md,verification-2026-07-round3.md}, decisions.md, agent-tasks/*, README.md; untracked: sprints/ (139), scratch/parse_trace.py, benchmarks/results.jsonl, benchmarks/model_profiles.json
- **Commit:** pending


## T-10601 (sprint 106)
- **Description:** Full environment-variable audit (`docs/environment.md`). Six runtime reads; the model can reach none of them (`.ferric` is read- and write-denied, the user config is outside containment, no `env::set_var` anywhere). **The real find was not an env var:** `Config` holds `api_key` in plaintext and **derived `Debug`**, putting the credential one `{:?}` from a log line or an `assert_eq!` failure. Now a hand-written `Debug` rendering it `<redacted>` while keeping presence visible. `BackendOpts` and `OpenAiConfig` carry the key and implement no `Debug`; that absence is now a **compile-time** assertion in each crate (confirmed by planting a derive and watching the build fail), each with a positive control so a detector that always answered "not Debug" cannot pass. **The sprint's own research report had claimed all three lacked `Debug`** — read off a `grep -B4` that stopped one line short — and the test with its positive control caught it; corrected in place. **env→config→hooks answered with disclosure, not lockdown:** `LoadedConfig.hooks_source` names the config file that supplied hooks (following the merge rule, so the file named is the one whose hook actually runs), and a run with hooks in effect prints it. Also gave `trace verify`'s temp file a pid-unique name (was a constant `verify.jsonl` in a shared world-writable dir — pre-creatable by another user, and a race between concurrent verifies). Scanned tree **and all history** for bearer credentials (`tskey-`, `ghp_`, `github_pat_`, `sk-`, `AKIA`, PEM): zero. 6 new tests.
- **Completed:** 2026-07-26
- **Files modified:** crates/ferric-cli/src/{config.rs,query.rs,trace_verify.rs}, crates/ferric-provider/src/openai.rs, docs/environment.md (new), decisions.md, agent-tasks/*, README.md
- **Commit:** pending


## T-10701 (sprint 107)
- **Description:** `--tier` / config `tier`: an explicit operator tier override (ADR-098), raised by the user against ADR-006 — before this, the only manual route to a tier was misstating `--params-b`, which is a *fact* about the model (and an input `model_profiles.json` and the ADR-029 read-back both trust), and afterwards nothing in the record could tell an earned tier from a claimed one. New `TierSource` (`Measured`/`Params`/`Override`) lands on `RunPolicy` itself — provenance is part of the decision, not metadata — and is written to `PolicySelected.tier_source`, rendered by `trace cat` as `Large (from override)`, and announced on stderr when used. Fieldless on purpose so it round-trips through the trace's one-word label (an earlier `Measured(u8)` draft could not; the level already lives in the profile store). `trace verify` **restores** the source rather than re-deriving it, so a re-run cannot relabel an override as measured. Wired on every surface that already exposes `--max-ring` (query/chat/mcp/icm/api), which was the existing precedent. `policy_for` unchanged for all existing callers. 5 new tests, including the one that states the point: an earned Large and an asked-for Large have identical budgets, so the label is the only distinguishing field. Old traces default to `"params"`, verified against a real pre-sprint-107 trace whose line lacks the key. Verified live: 1.2B → Nano/params; `--tier large` → Large, max_turns 40, override + stderr notice; config `tier = "medium"` persists.
- **Completed:** 2026-07-26
...
## T-10901 (sprint 109)
- **Description:** Research sprint, no code change: read OpenAI's Codex (github.com/openai/codex, Apache-2.0, ~101.7k stars) from crate listings and Rust source via the GitHub API, to deduce design decisions applicable to Ferric. ADR-100 + full research report. **Key structural finding:** Codex contains the agent at the **OS boundary** (Seatbelt / bubblewrap / Windows Sandbox; 3 sandbox modes x 3 approval policies; network denied by default) while Ferric contains it **in-process** in `ferric-guard` — so Codex can afford looser defaults because the kernel backstops them, and copying its posture without its sandbox would import the looseness and leave the backstop behind. **Settles the deferred `allowed-tools` question:** Codex's `SkillDependencies.tools` carries MCP coordinates (`transport`/`command`/`url`) and is a declaration of **need**, not a grant of **permission** — so Ferric can honour the key as a *precondition check* (refuse to compose a skill whose declared tools fall outside the active rings) without it becoming an action channel. **Validates ADR-091's stricter half by contrast:** Codex's `allows_implicit_invocation()` defaults to `true`; Ferric's `Authority` has no `Model` variant. **One concrete gap:** `execpolicy` has argv-token prefix rules, three-valued decisions, strictest-wins layering, and `host_executable()` absolute-path pinning, which Ferric has no analogue for; `check_command` is `contains()` over six strings and should at minimum be *described* as a footgun-catcher rather than a boundary. **Not adopted:** Codex has no constrained decoding (all 12 grammar hits are parsers) and no capability tiers — different target, not an argument against ADR-010/015/022; `code-mode` (embedded V8, model writes JS composing tool calls) is the antithesis of the constrained one-action-per-turn valve. **Outside corroboration:** their own `TODO: Enforce product gating ... instead of only parsing and storing this metadata` is the parsed-but-unenforced trap this project keeps finding, in a 101k-star repo.
- **Completed:** 2026-07-27
- **Files modified:** decisions.md, agent-tasks/agent-tasks.md, agent-tasks/completed-tasks.md
- **Commit:** pending
## T-11001/2/3/4/5/6 (sprint 110)
- **Description:** Hardened and rehearsed the Monday demo path. `trace verify` is now a side-effect-free structural transcript validator that never constructs a provider or executes recorded tools; its phase-aware checks accept real provider-failure traces and legitimate pre-dispatch guard stops while requiring exact native call counts for completed turns. CLI, MCP, and API attachments share workspace-relative read-policy enforcement, sensitive-path rejection, and 8 MiB per-file / 16 MiB aggregate limits. `shell_exec` and `manage_task` are absent from every model-visible ring while human chat passthrough retains guarded shell access; non-zero foreground exits are tool errors. `StopReason::is_success` is the single exhaustive success classification used by CLI, MCP, ICM, and loop error hooks. API mock mode now uses the mock provider, unauthenticated binds are loopback-only, and request bodies cannot replace the launch-fixed workspace. MCP mock mode reconstructs its finite script for every request, so successive `tools/call` requests remain independent. `launch` no longer prompts or starts an unrequested query after scaffolding. Repaired E2E scripts so native failures, partial artifacts, stale container entrypoints, or missing deletions fail the run, and added `tools/demo-smoke.ps1`: eight deterministic offline checks covering version, mock query artifact, trace inspection and safe verification, attachment denial, skills, launch, ICM, and cron. The smoke deletes the recorded artifact before verification and proves it is not recreated. Updated the README, environment audit, command reference, tool-ring reference, demo guide, and tooling guide to match the shipped CLI; the Monday guide pins the freshly built release executable and uses a harmless command-denial demonstration.
- **Completed:** 2026-08-01
- **Files modified:** README.md, decisions.md, agent-tasks/*, crates/ferric-cli/src/{api.rs,chat.rs,icm.rs,launch.rs,main.rs,mcp.rs,query.rs,trace_verify.rs}, crates/ferric-cli/tests/cli.rs, crates/ferric-loop/src/{outcome.rs,run.rs}, crates/ferric-tools/src/{lib.rs,builtin/mod.rs,builtin/shell_exec.rs,builtin/task_registry.rs}, crates/ferric-tools/tests/builtin_file_tools.rs, docs/{advanced-tool-rings.md,commands.md,demo-guide.md,environment.md}, tools/{README.md,demo-smoke.ps1,e2e_test.ps1,run-e2e.sh}, workspace/run-e2e-sweep.sh
- **Commit:** pending


## T-11101 (sprint 111)
- **Description:** Replaced the offline Monday acceptance criterion with a live server-backed gate at the user's direction. Built and used the repository's backend-enabled release binary; preflighted the installed `llama-server` and Qwen2.5-Coder-7B GGUF; launched through `ferric server up`; independently matched the registered PID to the `llama-server` process and loopback listener; received HTTP 200 from `/health` and `/v1/models`; ran the non-mock PowerShell E2E task through six constrained turns; asserted the generated two-argument addition function, requested directory deletion, `task_complete` trace terminator, and side-effect-free trace verification; then proved `server down` removed the process, listener, and both runfiles. Updated the Monday guide so mock smoke is a development check rather than demo acceptance. Recorded that the current llama.cpp distribution appears CPU-only and the validated task took about 58 seconds.
- **Completed:** 2026-08-01
...
- **Description:** Reconciled the landed typed navigation, controlled mutation,
  syntax-admission, and measured-effect work against the locked EARS clauses.
  Existing tests cover deterministic observation envelopes, stale and
  byte-identical refusals before approval, fail-closed opaque operations,
  bounded syntax transitions, exact content effects, and measured structural
  create/delete/move/copy effects. `cargo test -p ferric-tools`, affected
  Clippy with warnings denied, and the workspace format check all passed.
- **Intent:** [INT-0001](../intents/INT-0001-evidence-bound-autonomous-recovery.md)
