...
The accepted explicit-budget increment preserves the prepared-host human front
door: ordinary `cargo r` still has the same four primary actions and meaningful
decision budget. New output/timeout controls live on expert query/benchmark
routes and do not add a setup questionnaire, expensive mandatory calibration
or tool authority. T-11505 is reconciled; T-11509 still owns integrated
acquisition/calibration and complete resumable workflow state.

The first canonical Windows checkpoint failed with an unrecorded native cause.
Test-only instrumentation retains future inspection/session details; independent
review separately demonstrated and fixed a connection-reset defect in the
source fake engine, with a failing mutation control and unchanged deadlines.
Corrected-source native/CI/live evidence passed, but this is not a production
ownership repair or an explanation of the old failure. T-12111 retains that
attribution boundary; T-12026/27 retain native admission/parallel-load work.
A new canonical recurrence remains a blocker, not a rerun-to-green opportunity.

Existing WSL supplied additional process-free core evidence without changing
sudo permissions or bypassing the isolated native runner. It does not satisfy
ordinary-host Linux visibility, abrupt nested-group ownership or broader
platform parity (T-11707/T-11904). Source-owned cleanup remains mandatory and
the whole intent stays active; no state transition is appended for this progress.
