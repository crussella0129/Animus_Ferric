...
- [~] **Agent Delegation Structure (ICM):** Build a `ferric-icm` orchestrator crate that orchestrates sub-agents entirely via sequential local folders (`01_research/`, `02_script/`) and their `CONTEXT.md` files. *(Sprint 73, ADR-064 — increment 1: `ferric-icm` crate (contract parse, discover, guard-checked layered composition → `OrchestrationPlan`, refuse-to-clobber scaffolder) + `ferric icm init`/`plan`. Live per-stage loop execution with review gates = increment 2. See `docs/icm.md`.)*
