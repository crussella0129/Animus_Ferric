...
## ⚠ HANDOFF — Sprint 43 Test Phase is stuck mid-critique (2026-07-11)

**DELETE THIS ENTIRE SECTION once you have assessed and actioned it.** It exists only to hand off
exact state across a session boundary — it is not a permanent backlog item and must not accumulate.

**Where things stand right now:**
- Sprint 43 (Animus Launch inc 1, ADR-053) **Build Phase is fully done**: all 5 commits
  (`0093658`..`70b6486`, T-4301–T-4305) are on `dev`, working tree clean, `dev` has been **pushed to
  `origin/dev`** (verify with `git log origin/main..dev --oneline` — should show exactly those 5
  commits, nothing else).
- **No PR has been opened yet.** Nothing has been merged. `agent-tasks.md`'s own "Sprint 43 ...
  DONE" section below and `README.md`'s Sprint 43 entry were written optimistically during the
  build/docs task (T-4305) — they read as if the sprint is fully closed, but **the Test Phase never
  actually finished**, so treat "DONE" in those two places as aspirational until you close the loop
  below.
- `sprints/s43/sprint-tests/{unit-tests.md,integration-tests.md,e2e-tests.md}` are written and
  describe real, already-passing test runs (`cargo test -p animus-launch`: 4 unit + 5 integration;
  `cargo test -p ferric-cli`: 72 unit + 35 CLI subprocess incl. 3 new `launch` tests; workspace
  green; clippy clean; fmt clean — these numbers were independently run, not guessed).
- `sprints/s43/sprint-tests/test-report.md` **exists but is empty** (0 content) — never finalized.
- `sprints/s43/sprint-tests/critique.md` **does not exist** — the test-critic pass never completed.

**Why it's stuck:** a foreground test-critic agent was spawned (in a now-closed session; its agent
ID was `aefb5e423119e03db`, almost certainly not resumable from a fresh session) to independently
verify the claimed test counts, probe whether `animus-launch`'s refuse-to-clobber safety property
(`target_is_clobber_safe` in `crates/animus-launch/src/lib.rs`) is airtight against edge cases NOT
covered by the existing 3 clobber tests, verify the git command sequence halts correctly on a failed
`git commit` rather than silently proceeding to `git branch`, and run the full happy-path scaffold
against the real built binary. The agent's background run completed (27 tool uses, ~7 minutes — real
work was clearly done) but the delivered result contained no actual findings, only a boilerplate
safety-classifier-unavailable notice immediately followed by a session-limit cutoff. **The critic's
actual verdict — clean, or specific concerns — was never recovered and is unknown.**

**The two edge cases the critic was explicitly asked to probe (still unverified by any test):**
1. A **symlink** at the target path pointing at a non-empty directory — does `target_is_clobber_safe`
   correctly refuse it, or does `path.is_dir()`/`read_dir()` follow the symlink into false-safe
   territory (or false-refuse an empty-dir symlink)?
2. The **current directory (`.`)** as the target path when the cwd is non-empty (e.g. running
   `ferric launch` from inside an existing project by mistake) — same question.

**What to do next (in order):**
1. Read `crates/animus-launch/src/lib.rs`'s `target_is_clobber_safe` and reason about the two edge
   cases above directly, OR spawn a fresh test-critic-style review (`Agent` tool, foreground) with
   the same brief (test-count verification + the two edge cases + the git-halt-on-failure check + a
   real happy-path run against the built binary). Either is fine — the point is getting a real
   verdict, not literally resuming the dead agent.
2. Add tests for any real gap found (most likely: a `#[cfg(unix)]` symlink-to-nonempty-dir test in
   `crates/animus-launch/tests/scaffold.rs`, using `std::os::unix::fs::symlink` — Windows symlink
   creation needs elevated privileges so a Unix-gated test is the pragmatic choice; a `.`-as-target
   test can run on both platforms).
3. Write `sprints/s43/sprint-tests/critique.md` (findings + resolutions, even if the answer is
   "verified clean, no changes needed") and fill in the empty `sprints/s43/sprint-tests/test-report.md`.
4. Proceed to the Loop Phase per the locked plan (`sprints/s43/sprint-plans/` /
   `C:\Users\charl\.claude\plans\hashed-singing-hamster.md`): `dev` is already pushed, so open the
   sprint-43 PR, verify its commit count is exactly 5, watch CI to green, then **use
   `AskUserQuestion` to get explicit user confirmation before merging** (standing project rule — do
   not merge without asking, even though this is routine). After merge, recreate `dev` from the
   updated `main`.
5. Update `sprints/s43/sprint-meta.md` (End timestamp, Exit status: success) as part of closing.
6. **Delete this handoff section from `agent-tasks.md`** — its job is done once step 4 completes.
