...
- [ ] **F1 — the guard family has an A-B-A-B oscillation hole.** Live, qwen2.5-coder-7b alternated `search_files` / `find_files` for the **full 20-turn budget**: 20 tool calls, **2 distinct (name,args) pairs**, **zero guard events**. All three guards key on *consecutive-turn* state and reset on alternation — RepetitionGuard (name+args) and ProgressGuard (per-turn name set) both see a different turn each time, and FailureGuard never fires because the calls SUCCEED. Bounding wasted compute is the guard family's whole stated purpose (ADR-037/038). Proposed fix: a windowed guard over the last N turns (multiset of (name,args)) rather than a consecutive streak, so a 2-cycle is caught; keep the existing guards for the fast paths.
- [ ] **A1 is still not validated live.** The truncation cap was not exercised: qwen chose to paginate `read_file` via `start_line`/`end_line`, and `search_files` returned only 191 chars. The 4,000-char cap is reachable mainly via a file with very long *lines* or a large `shell_exec` (whose own cap is 10,000 — above the model cap). Needs a targeted live case.
