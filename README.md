...
Active development (sprint 40). Two inference backends ship behind feature flags:
...
> **Next — TBD.** Open: the new raw **chat mode** reversing ADR-011 (needing its own dedicated security-boundary ADR); MCP streaming notifications; `ferric mcp --resume`; install a containerizer → the Web retriever; wire the CaMeL sink policy once the research→loop taint source exists; per-tier compaction tuning; a hard truncation backstop for pathologically large first folds.
