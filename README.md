...
- **Sprint 44 — Event-Sourced Projector Refactor** (2026-07-13). Phase 1 of unblocking the Ornstein Web Retriever (increment 4). Extracted the duplicated state machine logic from `run.rs` and `replay.rs` into a pure `TraceProjector` struct in `projector.rs`. This correctly decouples protocol formatting from event hydration and fixes trailing-turn tracking issues in `compact.rs`. All tests pass cleanly, ensuring a hardened foundation for the microVM web retriever next. *ADR-054.*
