...
Active development (sprint 39). Two inference backends ship behind feature flags:
...
> **Next — Sprint 40: context-budget compaction** (model-driven summarization of older turns as a session nears its context budget, architecturally similar to `ferric-research::summarize_quarantined`'s constrained/no-tools shape but NOT a literal reuse — Ornstein's summarizer is for *untrusted* content, compaction summarizes the agent's own *trusted* history). Also open: the new raw **chat mode** reversing ADR-011 (needing its own dedicated security-boundary ADR); MCP streaming notifications; `ferric mcp --resume`; install a containerizer → the Web retriever; wire the CaMeL sink policy once the research→loop taint source exists.
