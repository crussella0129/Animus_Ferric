...
- **Sprint 66 — Async shell_exec Refactor** (2026-07-16). Refactored `ferric-tools`'s `shell_exec` command to use `tokio::process::Command` under `tokio::task::block_in_place`, ensuring runaway processes no longer block the async executor thread or cause disk/OOM thrashing via unbounded pipes.
