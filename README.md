...
Active development (sprint 37). Two inference backends ship behind feature flags:
...
> **Next — Sprint 38: TBD** (the new raw **chat mode** reversing ADR-011 — the revision's other half, needing its own dedicated security-boundary ADR; or a **Production-Readiness Roadmap** item — session resume via JSONL replay, or persistent config — see `agent-tasks/agent-tasks.md`; MCP streaming notifications; install a containerizer → the Web retriever; wire the CaMeL sink policy once the research→loop taint source exists).
