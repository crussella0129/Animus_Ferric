...
Active development (sprint 41). Two inference backends ship behind feature flags:
...
> **Next — Sprint 42: raw chat mode** (the ADR-011 revision's unbuilt half, deferred from sprint 41 by the user's scope choice — needs its own dedicated security-boundary ADR). Also open: operationalize the `ferric-core` container (run it end-to-end with a mounted model) + multi-arch images; the Ornstein Web retriever + microVM-sandbox airlock (now unblocked — Docker is installed, ADR-051); MCP streaming notifications; `ferric mcp --resume`; wire the CaMeL sink policy once the research→loop taint source exists; per-tier compaction tuning; a hard truncation backstop for pathologically large first folds.
