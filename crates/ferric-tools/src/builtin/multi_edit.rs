...
/// **atomically**: every edit is validated and applied to an in-memory working
/// copy first, and the file is written **once** only if all succeed — so a
/// failing edit leaves the file byte-identical. Ring 2 ("plan & apply structured
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            description: "Apply an ordered list of first-occurrence replacements to one file, \
                atomically (all-or-nothing). Args: {\"path\": string, \"edits\": [{\"old_string\": \
